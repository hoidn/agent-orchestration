...
**Status:** reviewed execution plan; Task 1 is selected after this plan-gate
commit. The review baseline is `19a98c8b`.
`M2_FEASIBILITY_PLAN_SPEC_APPROVED` then
`M2_FEASIBILITY_PLAN_QUALITY_APPROVED` approved the corrected candidate. M2
remains incomplete and M3a remains unselected until Task 4's executable proof
and ordered closure reviews approve the resulting design.
...
- [ ] Author one generic compiled workflow with this observable spine:
  ...
  out of the fixture.
- [ ] Build a test harness through the public compiler, ordinary
...
- [ ] Add a passing historical-profile control that captures:
...
- [ ] Prove the historical control still creates and reuses its ordinary pure
...
- [ ] Keep the green characterization in the Task 1 candidate; do not commit a
...
- [ ] Add RED cases proving the compiled fixture's validated compatibility
  ...
  contract preserved.
- [ ] Add both-direction negatives for unknown field/member/scope, duplicate
...
- [ ] Assert byte-identical serialized executable IR and runtime-plan
...
- [ ] Implement one immutable, identity-neutral in-memory index:
...
- [ ] Keep the executable and runtime-plan serializers byte-unchanged. Do not
  treat positional `dependencies` as dataflow and do not persist the derived
  index.
- [ ] Map all index/classification failures to
...
- [ ] Run the focused selector, the two complete owning modules,
...
- [ ] Classify every visit/cursor/row combination into unstarted, interrupted,
  derived-complete, durable failure/skip, or
  `progress_witness_invalid`.
- [ ] Run focused and complete owning modules, `git diff --check`, obtain
...
- [ ] Add RED clean-run assertions that an explicitly initialized
  replay-profile run:
...
- [ ] Add RED mixed-profile cases for a value-bearing successful eligible row,
...
- [ ] Add historical both-direction controls: valid old row/bundle reuse stays
...
- [ ] Run:
  ...
  bundles.
- [ ] Move the closed profile/witness/forbidden-surface audit after ordinary
...
- [ ] On exact replay profile, branch before pure bundle path allocation,
...
- [ ] Failed and skipped pure results remain ordinary full durable rows.
  Noneligible, recurrent, loop-owned, and historical pure results stay on the
  current durable path.
- [ ] Keep replay algorithms out of `executor.py`; the executor coordinates
  audited results and calls the small replay module.
- [ ] Keep this green suppression slice in the Task 3 candidate and proceed
...
- [ ] Add the RED main spine: interrupt E2 after committed E1 and settled B,
...
- [ ] Add RED interrupted-current spines for both A and B. Each has one
...
- [ ] Add both directions for checkpoint filtering:
  ...
  Never scan past it.
- [ ] Add both directions for `VALIDATED_FRAME_ENTRY_REPLAY`: admit only a
...
- [ ] Add invalid replay-source cases for missing/invalid bound input, missing
...
- [ ] Add an inactive-branch case proving an eligible projection outside the
...
- [ ] Select the restart boundary exactly once from audited witness states and
...
- [ ] Pass one profile-filtered checkpoint candidate tuple through
  ...
  record validation.
- [ ] Activate the validated restore overlay first, reconstruct the exact
  ...
  authority.
- [ ] Implement `VALIDATED_FRAME_ENTRY_REPLAY` as the one closed zero-prior
  case, not as a fallback past invalid durable authority.
- [ ] Keep this green resume slice in the Task 3 candidate and proceed
...
- [ ] Add a fresh non-iterative call-frame classification control without
...
- [ ] Add a read-only existing-frame loader/audit seam and invoke it from call
...
- [ ] Prove every loop/recur body, multiple-visit projection, and
  iteration-owned call frame retains full durable results and ordinary
  checkpoint behavior.
- [ ] Prove status and persisted-state summaries expose the exact completion
  shell, never a reconstructed value, while public artifacts, diagnostics, and
  settlement expose the ordinary final values.
- [ ] Prove absent-profile state and existing nested frames retain current
...
- [ ] Implement only the missing generic audits and classification seams.
  Positive nested replay activation remains for M3a; do not add a second
  fixture-only profile path.
- [ ] Run every Task 3 focused and complete owning module,
...
- [ ] Run collection for every new or renamed test module:

  ```bash
  pytest --collect-only -q tests/test_workflow_lisp_pure_result_replay.py
  ```

- [ ] In tmux, run the full M2 feasibility matrix with the repository-required
...
- [ ] Compare the replay-profile fixture with Task 1's historical control.
...
- [ ] Run roadmap/routing verification:

  ```bash
  pytest -q tests/test_workflow_lisp_drain_roadmap_routing.py
  ```

- [ ] Run one ordinary Workflow Lisp compiler/runtime smoke check in addition
...
- [ ] If every executable criterion passes, prepare the complete closure
...
- [ ] Re-run `git diff --check`, the complete focused matrix, and the routing
  selector against those complete closure bytes.
- [ ] In tmux, run the repository-standard broad non-security suite against
