...
- Modify: `workflows/library/prompts/lisp_frontend_design_delta_work_item/classify_blocked_implementation_recovery.md`
  - Clarify that `TERMINAL_BLOCKED/user_decision_required` is only for true human authority outside repo-local design/gap/prompt/contract/code repair.
...
## Task 5: Align Workflow YAML Contracts
...
## Task 6: Align Prompts With User-Input-Only Terminal Policy
...
```text
Use TERMINAL_BLOCKED/user_decision_required only when the evidence says no repo-local target design, gap design, workflow, prompt, contract, code, or prerequisite revision can safely choose the next action. Selector uncertainty, missing prerequisite representation, malformed artifact evidence, and implementation scope mismatch are recoverable and should route to target/gap/prerequisite recovery.
```
...
```bash
pytest tests/test_lisp_frontend_autonomous_drain_runtime.py -k "blocked_recovery or prerequisite_recovery or prerequisite_pending or terminal_blocker or implementation_blocker or user_decision" -q
```
...
- `TERMINAL_BLOCKED/user_decision_required` is preserved only with explicit evidence that repo-local recovery cannot choose the next action.
