...
|---|---|---|
| 1 | `lisp_frontend_design_delta_drain.yaml` (+ 6 `v214` library imports) | Promotion handoff recorded and Phase 3 Tasks 3.1–3.4 are complete with the historical report preserved. Gates P3 and P4 are independently reviewed and satisfied; `.orc` is primary, YAML/`v214` archive remains deferred to Stage 6, and the current selector is drain Phase 4 Task 4.1: strip design-delta constants and lanes from migration parity. Task 4.1 has not started, and no Task-4.1 deletion has begun. Stage 5 typed result guidance and Stage 6 YAML archive remain later work. |
