...
`848ceb52`, leaving 22 procedure candidates, 10 effect adapters, and 63
legacy-retire rows, plus eight separate public entries and one history row.
Task 5 Step 1 is current. After Tasks 5–8, continue with
...
8. **Effect-adapter evidence wave — complete.** Task 4 audited all 25 YAML
   ...
   `848ceb52`. The final queue is 22 procedure candidates, 10 effect adapters,
   and 63 legacy-retire rows, plus eight separate public entries and one
   history row. No YAML was translated or modified by this classification
   wave.
9. **Production-family wave — current selector.** Execute Task 5 Step 1 now,
   then Tasks 5–6 in their existing order with computed output, artifact,
   effect, resume, and checkpoint parity.
