...
- Behavioral evidence commands:
  ...
  - `python -m pytest tests/test_workflow_lisp_phase_stdlib.py -k resume_or_start or union_start_workflow_call -q` -> exit `0`; `17 passed, 86 deselected in 0.99s`
  - `python -m pytest tests/test_workflow_lisp_resource_stdlib.py -k finalize_selected_item -q` -> exit `0`; `7 passed, 27 deselected in 0.81s`
  - `python -m pytest tests/test_workflow_lisp_drain_stdlib.py -k backlog_drain or run_item_contract or providers_rebinding -q` -> exit `0`; `25 passed, 38 deselected in 7.40s`
  - `python -m pytest tests/test_lisp_frontend_autonomous_drain_runtime.py -k selected_item_fresh_plan or selected_item_reuses_approved_plan -q` -> exit `0`; `2 passed, 147 deselected in 3.20s`
  - `python -m pytest tests/test_neurips_steered_backlog_runtime.py -k drain_continues_to_next_iteration or drain_gap_draft or drain_blocked -q` -> exit `0`; `3 passed, 6 deselected in 4.70s`
