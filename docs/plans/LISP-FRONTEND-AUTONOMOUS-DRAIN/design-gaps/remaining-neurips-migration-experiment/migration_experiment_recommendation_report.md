...
| Shell/Python glue surfaces | 89 | 7 | Pass |
| String status/gate patterns | 116 | 3 | Pass |
...
- Behavioral evidence commands:
  - `python -m pytest tests/test_workflow_lisp_stage7_translation.py -k neurips_plan_gate_resume or neurips_selected_item or neurips_remaining_drain or run_item_boundary -q` -> exit `0`; `8 passed, 1 deselected in 0.60s`
  - `python -m pytest tests/test_workflow_lisp_phase_stdlib.py -k resume_or_start or union_start_workflow_call -q` -> exit `0`; `17 passed, 86 deselected in 1.06s`
  - `python -m pytest tests/test_workflow_lisp_resource_stdlib.py -k finalize_selected_item -q` -> exit `0`; `7 passed, 27 deselected in 0.85s`
  - `python -m pytest tests/test_workflow_lisp_drain_stdlib.py -k backlog_drain or run_item_contract or providers_rebinding -q` -> exit `0`; `25 passed, 38 deselected in 7.36s`
  - `python -m pytest tests/test_lisp_frontend_autonomous_drain_runtime.py -k selected_item_fresh_plan or selected_item_reuses_approved_plan -q` -> exit `0`; `2 passed, 147 deselected in 3.35s`
  - `python -m pytest tests/test_neurips_steered_backlog_runtime.py -k drain_continues_to_next_iteration or drain_gap_draft or drain_blocked -q` -> exit `0`; `3 passed, 6 deselected in 4.88s`
