...
- Behavioral evidence commands:
  - `python -m pytest tests/test_workflow_lisp_stage7_translation.py -k neurips_plan_gate_resume or neurips_selected_item or neurips_remaining_drain or run_item_boundary -q` -> exit `0`; `8 passed in 0.20s`
  ...
  - `python -m pytest tests/test_lisp_frontend_autonomous_drain_runtime.py -k selected_item_fresh_plan or selected_item_reuses_approved_plan -q` -> exit `0`; `2 passed, 14 deselected in 1.99s`
  - `python -m pytest tests/test_neurips_steered_backlog_runtime.py -k drain_continues_to_next_iteration or drain_gap_draft or drain_blocked -q` -> exit `0`; `3 passed, 6 deselected in 3.71s`
