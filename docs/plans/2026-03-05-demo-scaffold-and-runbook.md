...
Current flagship evaluator note:
- the nanoBragg hidden corpus is now generated by a broad-call-chain main-wrapper reference harness under `scripts/demo/nanobragg_entrypoint_reference/`
- the current per-case provenance snapshot is recorded in `orchestrator/demo/evaluators/fixtures/nanobragg_entrypoint/cases.json`
- the harness is maintenance-only and is not exposed inside either trial workspace

Current seed status:
- flagship seed: `examples/demo_task_nanobragg_entrypoint_port`
- bootstrap baseline seed: `examples/demo_task_linear_classifier_port`

## Seed Scaffold

The generic scaffold lives at [examples/demo_scaffold](/home/ollie/Documents/agent-orchestration/examples/demo_scaffold). Task-specific seeds such as [demo_task_nanobragg_entrypoint_port](/home/ollie/Documents/agent-orchestration/examples/demo_task_nanobragg_entrypoint_port) extend that shape for a concrete trial.
...
- the runner now accepts `--direct-model`, `--direct-effort`, `--workflow-model`, and `--workflow-effort`
- the direct-arm launcher shape is `claude -p ... --dangerously-skip-permissions --model <model> --effort <effort>`
- output streaming is enabled by default; pass `--no-stream-output` to suppress live console teeing

Seed-repo requirement:
- `run_trial.py` provisions from a git repository, not an arbitrary directory tree
- if the task seed lives under `examples/` as a plain directory, create a temporary git snapshot and use that path as `--seed-repo`
...
python scripts/demo/evaluate_nanobragg_entrypoint.py /path/to/frozen-workspace
```
...
- `port_nanobragg_entrypoint_to_pytorch.md` dispatches to `scripts/demo/evaluate_nanobragg_entrypoint.py`
- for the nanoBragg seed, hidden expected tensors are regenerated offline by `scripts/demo/build_nanobragg_entrypoint_cases.py`, which delegates to `scripts/demo/nanobragg_entrypoint_reference/run_reference_case.py`
...
1. Run a real trial against `examples/demo_task_nanobragg_entrypoint_port` and archive the result.
...
For the current nanoBragg flagship, the authoritative acceptance target is the extracted `nanobragg_run` entrypoint exercised by the hidden evaluator and the visible fixtures in `examples/demo_task_nanobragg_entrypoint_port/`; direct and workflow runs should be judged against that entrypoint's outputs rather than a prose-heavy public contract.
