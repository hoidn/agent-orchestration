...
- the nanoBragg hidden corpus is now generated by an offline C-backed reference harness under `scripts/demo/nanobragg_reference/`
- the current per-case provenance snapshot recorded in `orchestrator/demo/evaluators/fixtures/nanobragg_accumulation/cases.json` is `scoped-simplified-harness-v1`
...
- for the nanoBragg seed, hidden expected tensors are regenerated offline by `scripts/demo/build_nanobragg_reference_cases.py`, which delegates to `scripts/demo/nanobragg_reference/run_reference_case.py`
...
For the current nanoBragg flagship, the authoritative acceptance target is the scoped contract in `examples/demo_task_nanobragg_accumulation_port/docs/tasks/nanobragg_accumulation_contract.md`; direct and workflow runs should be judged against that narrower contract rather than a broader inferred nanoBragg subsystem.
