...
| Drain expression typecheck owner | **Retired in Task 2.3.** `typecheck_backlog_drain_expr` and `BacklogDrainExpr` are deleted. `orchestrator/workflow_lisp/typecheck_drain_phase.py` remains only for its generic phase-target/provider-phase responsibilities; imported `std/drain.orc` procedures now use the ordinary procedure/typechecking path. |
| Shared typecheck helpers and dispatch | **Retired in Task 2.3.** `workflow_ref_signature`, the three specialized workflow-ref validators, `_backlog_drain_blocker_class_type`, and the `BacklogDrainExpr` dispatch branch are deleted. `typecheck_calls.py` and `typecheck_dispatch.py` remain generic owners and contain no backlog-drain-specific dispatch. |
| Intrinsic drain lowering | **Retired in Tasks 2.1–2.2.** `lowering/phase_drain.py`, `lowering/drain_terminal.py`, `_callable_backlog_drain_*`, `_ensure_callable_backlog_drain_workflow`, `_phase_stdlib_lower_backlog_drain_impl`, `_validate_backlog_drain_provider_metadata`, `lower_shared_drain_terminal_result`, and their intrinsic-only helpers are deleted. The live drain body is the imported generic procedure/macro in `orchestrator/workflow_lisp/stdlib_modules/std/drain.orc`. |
