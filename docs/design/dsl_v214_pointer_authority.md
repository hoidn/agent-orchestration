...
| Library workflow artifact registry | `workflows/library/revision_study_design_phase.yaml` | canonical top-level artifact pointer | This workflow declares phase artifacts under `artifacts:` and then publishes same-step outputs through them. The artifact contract, not the local output name, owns the canonical pointer path. |
| Same-step publish source | `expected_outputs` plus `publishes.from` in the library workflow above | local step materialization pointer | The local output name is a same-step handle for publication. When it feeds a published relpath artifact, its pointer path must match the canonical top-level pointer or be omitted. |
