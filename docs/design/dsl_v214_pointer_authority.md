...
| Library workflow artifact registries | `workflows/library/revision_study_design_phase.yaml`, `workflows/library/neurips_backlog_seeded_plan_phase.yaml` | canonical top-level artifact pointer | These workflows declare phase artifacts under `artifacts:` and then publish same-step outputs through them. The artifact contract, not the local output name, owns the canonical pointer path. |
