...
| [Workflow Lisp evolution substrate and feature design](../superpowers/specs/2026-07-22-workflow-lisp-evolution-substrate-and-feature-design.md) | Compiler-certified immutable variants, context-stratified subject adjudication, inert fragment archives, role-separated prompt environments, prompt/code evolution, and the substrate/feature boundary | Draft design | No | Separates lexical prompt bindings, protected composition captures, per-attempt invocation evidence, and experimental background while keeping neutral subject/certification/trial/assessment substrate distinct from evolution policy. It does not add runtime `eval`, closures, recursive `defun`, fragment execution, or implemented evolution syntax. |
...
| [workflow_lisp_provider_live_binding.md](workflow_lisp_provider_live_binding.md) | Live provider supervision through observation panes, one bounded provider-session correction, and recorded peer messaging | Accepted design; 2026-07-24 owner amendment (recorded peer messaging, v1.1) pending amendment review | No | Adverse T3a retired *same-turn* `send-keys` steering only. v1 composes one worker and one supervisor with a validated directive and one fail-closed turn-boundary resume under a single-writer coordinator; the owner amendment adds recorded free-form peer messaging with explicit turn-boundary delivery and static N-member composition as v1.1. Roadmap execution remains gated on a reviewed Stage 7 plan. |
