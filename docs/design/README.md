...
| [workflow_lisp_transportable_value_type.md](workflow_lisp_transportable_value_type.md) | Opt-in transport-contract top `Value`, strict JSON wire validation, exact source compatibility, and target-gated public `type: value` / `kind: value` | Designed | No; Q0 implementation pending | Accepted target-2.19 prerequisite for prompt calculus under the active language-quality roadmap. `Value` is exact and opaque in source typing, distinct from non-transportable `Json`, and uses existing direct-root `__result__` carriage without an envelope. |
...
| [workflow_lisp_prompt_calculus.md](workflow_lisp_prompt_calculus.md) | Typed compositional prompt layer: defprompt fragments with kind-based slots (doc/text/value/path, optional refinement), completeness checking, fragment/result coherence, prompt identity, judgment views | Planned; Q1 blocked by Value Q0 | No | Selected by the active language-quality roadmap. Before implementation its design must remove procedure/provider substitution, close kind/delivery/diagnostic ownership, and narrow Q1 to fully applied fragments plus prompt-carried result derivation and the minimum fragment-program digest. Runtime prompt values and optimization remain excluded. |
...
| [workflow_lisp_language_server.md](workflow_lisp_language_server.md) | `.orc` LSP server (editor diagnostics and navigation) | Implemented | No; use the setup guide for editor configuration | Pure consumer of existing compile entry points per frontend spec §76.1. V1 is read-only clean-open/save diagnostics plus closed navigation under one canonical root; tolerance, hover, overlay, caching, and other P1–P5 work remain deferred. See [the setup guide](../workflow_lisp_language_server_setup.md). |
