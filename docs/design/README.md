...
| [workflow_lisp_post_foundation_composition_stdlib_migration.md](workflow_lisp_post_foundation_composition_stdlib_migration.md) | Post-foundation Workflow Lisp composition and stdlib migration | Draft follow-up target | No | Next target after the runtime foundation; use for nested structured-control composition, typed result translation, imported/std `.orc` reuse, review/revise stdlib convergence, private executable context, certified adapter/state-transition ownership, and parent-callable parity promotion. |
| [workflow_lisp_generic_resource_context_core.md](workflow_lisp_generic_resource_context_core.md) | Generic resource/context core for post-foundation Workflow Lisp | Draft simplification target | No | Narrows private context and resource-transition work to a small runtime core (`RunCtx`, `Resource<TState>`, `Transition<TRequest, TResult>`) with domain contexts such as `PhaseCtx`, `DrainCtx`, and `RecoveryCtx` expressed as library records. |
| [workflow_lisp_core_calculus_middle_end.md](workflow_lisp_core_calculus_middle_end.md) | Workflow core calculus and compiler middle-end | Draft future-direction architecture | No | Structural fix behind the composition gaps: one generic lowering route (calculus, ANF, join points, defunctionalization) replacing per-form lowerers; the architecture post-foundation Tranche 1 should be implemented against once accepted. |
