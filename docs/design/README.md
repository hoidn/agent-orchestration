...
| [workflow_lisp_prompt_calculus.md](workflow_lisp_prompt_calculus.md) | Target-2.20 prompt core plus target-2.21 output positions | Implemented | Yes for the bounded Q1 and Q2 surfaces | Q1 and Q2 are implemented through the real `review-design-docs` consumer with classic/WCC parity, schema-2.1 fragment snapshots, fail-closed v1/v2 identity carriage, compiler-owned required-file projection, state-atomic file-plus-structured validation, and compatible completed-boundary resume. The reviewed Q2 implementation record is `docs/plans/2026-07-26-workflow-lisp-prompt-output-positions-implementation-plan.md`; Q3–Q4 retain separate ownership, the Q3 design and reviewed implementation plan are accepted, and Q3 implementation is next. |
| [workflow_lisp_prompt_identity_diagnostics.md](workflow_lisp_prompt_identity_diagnostics.md) | Target-2.22 attempt-scoped, role-separated prompt identity and comparison diagnostics | Accepted design | No; implementation has not started | Accepted after ordered `Q3_DESIGN_SPEC_APPROVED` then `Q3_DESIGN_QUALITY_APPROVED` over immutable snapshot `fdf16f362f93eae89c05600e6954a118270fe7b7` and landed at `9b2aa7ac`. The [reviewed implementation plan](../plans/2026-07-27-workflow-lisp-prompt-identity-diagnostics-implementation-plan.md) is accepted at `ad5474c7`; implementation is next, and acceptance does not expose target-2.22 behavior or unblock Q4. |
...
| [workflow_lisp_language_server.md](workflow_lisp_language_server.md) | `.orc` LSP server (editor diagnostics and navigation) | Implemented | No; use the setup guide for current editor configuration | Pure consumer of existing compile entry points per frontend spec §76.1. V1 remains read-only clean-open/save diagnostics plus closed navigation under one canonical root; completed L0 adds content-keyed pure-projection export reuse, one-probe no-watcher reverse invalidation, structured initialization failures, and visible compiler notes/expansion roles. L1 authored symbols/signatures are implemented with compiler-owned ten-kind projection, exact ranges, and namespace-preserving callable details. L2 recovery-safe static completion is implemented through `10e3ccc3`: current success keeps the full L1 list, valid recovery receives only process-frozen form heads with no stale callable, and all closed branches remain empty; definition/document symbols stay success-only. Ordered `L2_FINAL_SPEC_APPROVED` then `L2_FINAL_QUALITY_APPROVED` close its reviewed implementation plan. Tolerance, hover, overlay, general compile caching, runtime debugging, and other P1–P5 prerequisites remain deferred. See [the setup guide](../workflow_lisp_language_server_setup.md) for shipped behavior. |
| [workflow_lisp_lsp_authored_reference_navigation.md](workflow_lisp_lsp_authored_reference_navigation.md) | L5 authored-to-authored definition navigation for prompt application heads plus the feasibility-admitted narrow direct-retained `proc-ref` shape | Accepted design (implementation-plan review pending) | No | Accepted at `b8a41172`; read-only gates admit prompt heads and only final unexpanded direct-retained proc-refs in non-generated, non-specialized authored owners. Macro heads defer shape-wide; macro-consumed/erased/generated-owner/specialized-owner proc-refs, new direct-call behavior, and WCC/generated calls remain excluded. The [proposed plan](../plans/2026-07-27-workflow-lisp-l5-authored-reference-navigation-implementation-plan.md) awaits ordered specification then quality review. |
