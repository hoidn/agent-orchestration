...
| [workflow_lisp_private_runtime_state_and_consumer_value_flow.md](workflow_lisp_private_runtime_state_and_consumer_value_flow.md) | Private runtime checkpoints and consumer-side typed-value rendering | Partial: Track R default resume implemented; broader cleanup/rendering target remains | No | Governing umbrella for two independent tracks. Track R includes the implemented fail-closed node-local and unique-nearest-prior default-resume contract; remaining resume-only plumbing retirement and Track C consumer rendering retain their documented target status. |
...
| [workflow_lisp_prompt_calculus.md](workflow_lisp_prompt_calculus.md) | Typed compositional prompt layer: defprompt fragments with kind-based slots (doc/text/value/path, optional refinement), completeness checking, fragment/result coherence, prompt identity, judgment views | Proposed direction (owner-directed; design review pending) | No | Head of the post-Stage-8 queue by owner direction. Unifies five shipped seeds (output-contract rendering, prompt dependencies, generic-reviewer pattern, runtime preludes, E4P identity roles). First tranche: defprompt + slot-discharge checking. Structural completeness only; no runtime prompt values; no optimization semantics; union-parsimony bound. |
| [workflow_lisp_pure_list_traversal.md](workflow_lisp_pure_list_traversal.md) | Target-2.18 list construction/traversal, pure and bounded-effectful mapping, list loop state, and rooted path joining | Accepted at `80df0616`; implementation planned | No, until implementation lands | Principle-29-compatible runtime-cardinality fan-out over whole-list-contract-expressible types; record/union elements, higher-order mapping, and broader effect bodies remain deferred. The independently reviewed implementation plan is the active post-Stage-7 interstage selector and has no Stage-8 dependency. |
...
| [workflow_lisp_language_server.md](workflow_lisp_language_server.md) | `.orc` LSP server (editor diagnostics and navigation) | Accepted at `cfcac27f`; implementation queued | No | Pure consumer of existing compile entry points per frontend spec §76.1; v1 is save-driven diagnostics plus navigation; tolerance/hover/overlay deferred behind named frontend prerequisites. Roadmap-scheduled as final Stage 8 of the procedure-first execution sequence; execute only through the reviewed Stage-8 implementation plan after the active list-traversal interstage closes. |
