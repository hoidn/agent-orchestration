...
| [workflow_lisp_lsp_authored_reference_navigation.md](workflow_lisp_lsp_authored_reference_navigation.md) | L5 authored-to-authored definition navigation for prompt application heads plus feasibility-gated direct-retained `proc-ref` and macro heads | Proposed (revised independent specification review pending, then quality review) | No | Not implemented; no implementation planning until both ordered reviews approve and the read-only retention gates decide the conditional subshapes. Macro-consumed/specialized `proc-ref` occurrences defer, macro hits may target only their own authored `defmacro`, direct `(call ...)` navigation is already implemented regression coverage, and WCC/generated calls remain excluded. |
