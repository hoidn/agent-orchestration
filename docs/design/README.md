...
| [workflow_lisp_prompt_calculus.md](workflow_lisp_prompt_calculus.md) | Target-2.20 prompt core: importable `defprompt`, closed doc/text/value/path slots, fully applied named fills, prompt-owned returns, and a minimum compiled-fragment identity | Implemented | Yes for the bounded Q1 surface | Q1 is implemented through the real `review-design-docs` consumer with classic/WCC parity, schema-2.1 fragment snapshots, fail-closed identity carriage, and compatible completed-boundary resume. Q1 forbids residual fragments, prompt values, parallel call-site inputs/dependencies/returns, output-position slots, judgments, and procedure/provider substitution. Q2–Q4 retain their separate ownership. |
