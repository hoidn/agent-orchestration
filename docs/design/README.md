...
| [Workflow Lisp evolution substrate and feature design](../superpowers/specs/2026-07-22-workflow-lisp-evolution-substrate-and-feature-design.md) | Compiler-certified immutable variants, expression-granular post-hoc adjudication, prompt/code evolution, and the substrate/feature boundary | Draft design | No | Separates neutral subject/certification/trial substrate from thin evolution admission and replaceable search policy. It does not add runtime `eval`, closures, recursive `defun`, or implemented evolution syntax. |
...
| [workflow_lisp_provider_live_binding.md](workflow_lisp_provider_live_binding.md) | Live provider supervision through observation panes and one bounded provider-session correction | Accepted design (implementation plan pending) | No | Adverse T3a retired direct `send-keys`. Revised v1 composes exactly one worker and one supervisor, uses a validated prelude directive, and permits one fail-closed turn-boundary resume under a single-writer coordinator. Independent specification and quality reviews passed; roadmap execution remains gated on a reviewed Stage 7 plan. |
