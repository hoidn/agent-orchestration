...
| [workflow_lisp_prompt_calculus.md](workflow_lisp_prompt_calculus.md) | Typed compositional prompt layer: defprompt fragments with kind-based slots (doc/text/value/path, optional refinement), completeness checking, fragment/result coherence, prompt identity, judgment views | Planned; Q1 design correction/review next | No | The target-2.19 `Value` prerequisite is implemented. The active language-quality roadmap selects Q1 design correction and ordered review next; before implementation the design must remove procedure/provider substitution, close kind/delivery/diagnostic ownership, and narrow Q1 to fully applied fragments plus prompt-carried result derivation and the minimum fragment-program digest. Runtime prompt values and optimization remain excluded. |
