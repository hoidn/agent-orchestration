...
| [workflow_lisp_runtime_native_drain_authoring.md](workflow_lisp_runtime_native_drain_authoring.md) | Runtime-native Workflow Lisp drain authoring | Draft target | No | Concrete acceptance target for a working Design Delta Drain `.orc` family shaped around typed provider request records, private runtime context, consumer-side rendering, typed projections, resource transitions, and certified adapter retirement. |
