...
| [workflow_lisp_lexical_checkpoint_resumability.md](workflow_lisp_lexical_checkpoint_resumability.md) | Lexical checkpoint resumability for Workflow Lisp | Draft future target | No | Future runtime target that separates execution resumability from workflow-domain resource state: restore private lexical checkpoints for continuations, bindings, loop/call frames, proof scopes, and effect-boundary evidence while keeping typed resources/transitions as the authority for durable semantic mutation. |
