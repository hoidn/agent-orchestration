...
| [workflow_lisp_language_server.md](workflow_lisp_language_server.md) | `.orc` LSP server (editor diagnostics and navigation) | Accepted at `cfcac27f`; implementation active | No | Pure consumer of existing compile entry points per frontend spec §76.1; v1 is save-driven diagnostics plus navigation; tolerance/hover/overlay deferred behind named frontend prerequisites. The target-2.18 list-traversal interstage is complete, so the reviewed Stage-8 implementation plan is the active execution-sequence selector. |
