...
| [workflow_lisp_native_transportable_returns.md](workflow_lisp_native_transportable_returns.md) | Uniform native returns and optional typed result guidance | Accepted target | No | Governs the two gated Stage-5 waves: native direct-root carriage first, typed root/field guidance second. Do not treat v2.15 as implemented until both completion gates pass. |
