...
\scriptsize
The compiler does not produce machine code or YAML text. It produces checked internal workflow objects for the existing executor.
\end{block}
\end{column}
\end{columns}
\end{frame}
...
\hline
Elapsed wall time inspected & 31.45 hours & 9.31 hours \\
Completed units & 12 design gaps & 75 Ralph iterations \\
Raw rate & 0.38 design gaps/hour & 8.06 iterations/hour \\
Approx. unique work items & 12 & 46 \\
Functional MVP completion & about 95\% & about 74\% \\
Functional full-design completion & about 82\% & about 24\% \\
\end{tabular}
\vspace{0.8em}
...
\begin{itemize}
  ...
  \item Changed-file ledger: 231 touches over 83 unique files; 18 files repeat.
  \item Hot spots: \texttt{test\_workflow\_lisp\_lowering.py} 42/60 iterations; \texttt{lowering.py} 40/60.
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
Ralph's hottest source file concentration is about 4.4x the full-drain report-mention proxy: 40 changed-file touches versus 9 mentions. Repeat surplus is 148 non-unique Ralph touches versus 48 full-drain repeated report mentions.
\end{block}
\end{frame}
