...
\begin{frame}{Why a Lisp frontend over YAML?}
\small
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.48\textwidth}
\begin{block}{YAML stays close to runtime}
\footnotesize
\begin{itemize}
  \item Explicit step wiring and migration/debug visibility.
  \item Existing executor still owns state, resume, ledgers, artifacts, and provider/command execution.
  \item Command steps remain valid for real external work.
  \item Debug YAML can help audit compiled workflows, but it is not compiler authority.
\end{itemize}
\end{block}
\end{column}
\begin{column}{0.48\textwidth}
\begin{block}{Lisp removes semantic glue}
\footnotesize
\begin{itemize}
  \item Typed records/unions replace markdown and JSON scraping.
  \item \texttt{match} proves variant-specific access instead of hand-pairing \texttt{when} and \texttt{requires\_variant}.
  \item Phase context replaces repeated state-path and pointer conventions.
  \item Procedures/macros reuse workflow meaning, not copied shell blocks.
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
\footnotesize
Not ``no Python.'' No hidden semantic Python. Inline Bash/Python is fine for incidental tooling, but workflow meaning should be a typed \texttt{provider-result}, \texttt{command-result}, certified adapter, or frontend/runtime form that lowers through shared validation and source maps.
\end{block}
\end{frame}
