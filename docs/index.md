...
**Description:** Current executable selector for post-hardening procedure-first family migrations. Task 1's rebaseline and Task 2 Step 1's fail-closed retention decision are complete; Task 2 Step 2 is current.
...
**Description:** Reviewed current inventory of 95 active internal authored call sites—31 procedure candidates, 26 effect adapters, and 38 legacy-retire sites—plus three separately recorded public entries and one append-only migrated-history row, with machine-readable provenance in the adjacent JSON file.
