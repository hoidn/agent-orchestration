...
**Description:** Stage 7 v1 contract, implemented through `4d4f05c7`, for default provider observation plus target-2.16 `with-live-providers`: exactly one worker and one supervisor run under a single-writer coordinator, and a validated `CONTINUE|STEER` union permits at most one fail-closed provider-session correction. The additive v1.1 contract is owned by the separate peer-messaging design.
