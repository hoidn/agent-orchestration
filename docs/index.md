...
**Use this when:** Checking the E-series shape, gates, and sequencing prerequisites, or tracing the historical proposal. The accepted designs remain non-copy-safe because their surfaces are not implemented. ML closure and the lean-pilot owner-decision handoff are complete. The owner has authorized E0 activation, but E0 remains unselected while its proposed component plan awaits ordered review; the handoff does not select E1+. E4P is exclusively absorbed by successor Stage Q3.

### [E0 Canonical Direct-Control Component Plan](plans/2026-07-31-workflow-lisp-e0-direct-control-component-plan.md)
**Description:** Proposed E0-only component plan for one target-2.23 library workflow with typed task/model/effort inputs, exactly one composed provider boundary, a direct `Bool` result, completed-result reuse, and accounting-parity conformance against an ordinary one-provider workflow.
**Keywords:** workflow-lisp, e-series, e0, direct-control, provider, accounting, component-plan
**Use this when:** Reviewing or, after ordered plan approval and explicit status selection, executing E0. The plan is currently proposed and selects no implementation. It creates no new DSL target, runtime, loader, state, trial, child-run, controller, or historical evolution-selector machinery, and cannot select E1+.
