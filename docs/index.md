...
**Description:** Proposed tracked program (2026-08-14) for integrating the omp harness as a layer swap: one generic `omp` provider template, an `OMP_JSON_STDOUT` session codec feeding the existing run-scoped tmux observation panes, a prompt+conf scaffolder that generates and runs ordinary `.orc` workflows through the shared `workflows/library/omp/` procedure, named multiagent conf presets (`advised/`, `fanout/`, `peer-team/`, `advised-fanout/`), and a bidirectional session bridge (`omp --resume` on run evidence; session import back into orc). It extends and corrects the [omp integration proposal](plans/2026-08-14-omp-integration-proposal.md) (G1–G10 grounding table); the RPC interactive lane is consumer-gated; memory and cross-step hub state are banned by invariant.
**Keywords:** omp, provider, harness, session-transport, codec, scaffolder, multiagent, advisor, fanout, observability, tmux, roadmap
**Use this when:** Reviewing or executing the omp integration tranche, choosing a multiagent conf preset, or auditing the admission tiers and hermeticity contract. Nothing is selected by listing: tranche 1 requires the owner approval recorded in the extension's Open Questions, feasibility gates F1–F6 are exit criteria for its first run, DP-A (2.16/2.17 deprecate-or-commit, drafting-guide guidance) waits for trial evidence, and nothing executes against the frozen ES apparatus.
