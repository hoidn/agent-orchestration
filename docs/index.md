...
**Use this when:** Executing or auditing E2 under the exact reviewed candidate at `c6046d38`, tree `40c533fc`, after ordered `E2_PLAN_SPEC_APPROVED` then `E2_PLAN_QUALITY_APPROVED`; see the [plan review](../artifacts/review/e2-trial-plan-review.md). Tasks 1–4 are complete at `456acc7a`, `6b431087`, `43ae8d5c`, and `ba430ed2`, tree `dd649f39`; Task 5 IR/lowering/persistence/checkpoint carriage is selected, while no E2 trial runtime producer exists yet. The historical registry/handle E2 proposal, E2O, C1-C3, and security/isolation work are not selected by this plan.
...
**Description:** Tracked recursion/ergonomics program (2026-08-01): REC0 current-surface residual measurement (authoring-only), REC1 fuel-bounded self-call as compile-time unrolling under binding forward-compatibility constraints, REC1b call-argument record spread, Gate REC over any dynamic-recursion future, REC2 as an unscheduled horizon marker, and REC2' run-boundary unboundedness as the recorded alternative terminal. No REC item is selected by listing.
**Keywords:** workflow-lisp, recursion, bounded-iteration, loop, ergonomics, frontend, rec-series, roadmap
**Use this when:** Checking REC tracking, ordering, entry conditions, or gates. REC0 is selectable at any time (authoring/report only); REC1/REC1b require E-program completion or explicit owner acceleration with a reviewed sequencing amendment, plus a design amendment and a reviewed component plan; REC2 enters planning only through Gate REC's `PROCEED_TO_REC2`. Item IDs use `REC` because `R1`/`R2` name historical experiment-design tasks.
