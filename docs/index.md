...
**Description:** Active post-Stage-8 selector with two bounded series: Q0–Q3 are implemented and Q3 closes under its reviewed plan; Q4 still needs its concrete consumer, while Q5 retains its separate gates. In the parallel L-series, L0 reliability/actionability, L1 authored symbols/signatures, L2 recovery-safe static completion, L3 immutable per-source entry selection, and owner-reordered L5 authored reference navigation are complete. MR-4 closed L3's compile-path-reentrancy prerequisite before implementation. L4 diagnostic lifecycle and compile progress has accepted editor evidence and ordered design reviews; its reviewed implementation plan is accepted and pre-implementation controls are next.
...
**Use this when:** Selecting or sequencing current post-Stage-8 work. Q4 remains gated on a bound concrete consumer. In L, L3 completed over MR-4 under its reviewed plan; L4's reviewed implementation plan is accepted and pre-implementation controls are next. Do not select E0, the parked evolution roadmap, shelved parsimony candidates, or P1–P5 from this route.
...
**Description:** Accepted target-2.23 Stage-Q5 design at `872a29af` for explicit phased delivery of one canonical fragment prompt: it preserves compiled Q1/Q2 identity, consumes completed Q3 attempt evidence with no Q4 dependency, and gives a dedicated single-attempt coordinator exact canonical-versus-actual identity/reporting, a proof-authoritative closed start outcome with one handle-free provider-cleanup evidence union, exact active-handle validation before projecting the unchanged target-2.17 adapter proof, inert pre-start binding/locator values and post-start endpoint binding, one deadline, full Q2 validation, T2 cleanup-pending/finished terminalization, natural-proof-before-ledger joined-pending-commit publication, total reason-equal diagnostics, and ledger-only digest validation. The implementation plan is reviewed at `45468c55`; P0 characterization then P1/P2 prerequisites are next.
**Keywords:** lisp-frontend, prompts, phased-delivery, target-2.23, provider-attention, principle-30, turn-boundary
**Use this when:** Reviewing Q5’s accepted additive call policy, Q3/report version gate, closed-outcome deadline-aware production-adapter prerequisite, capability refusal, byte cut, materialization retry, handle-bound-proof versus handle-free-evidence projection, pre-start zero-resource and post-start endpoint-allocation boundary, T0–T4 failure grammar, diagnostic projection, offline digest categories, endpoint shutdown, post-join failure, or sticky interruption contract. Implementation not started; explicit `:delivery :phased` remains unavailable and fail-closed. P0 characterizes the prerequisite gap before P1/P2 implement and prove that shared substrate. Coordinator invalid-then-valid and terminal-resource behavior remain later implementation gates. The separate `prompt-queue` proposal is not a dependency.
...
**Description:** Reviewed preimplementation plan committed at `45468c55` after ordered `Q5_PLAN_SPEC_APPROVED` then `Q5_PLAN_QUALITY_APPROVED`. It sequences P0 characterization, then P1/P2 shared adapter prerequisites, a fresh post-prerequisite control, and the later Q5 implementation tasks.
**Keywords:** lisp-frontend, prompts, phased-delivery, implementation-plan, target-2.23, q5
**Use this when:** Executing or auditing Q5’s reviewed order and ownership. Q3 is complete and Q5 has no Q4 dependency; P0 characterization then P1/P2 prerequisites are next. Implementation not started; `:delivery :phased` remains unavailable until the plan’s implementation, normative, evidence, and ordered-final-review gates close.
...
**Description:** Accepted L4 target for a current-only diagnostic publication
...
**Use this when:** Reviewing or planning L4. The target has ordered
`L4_DESIGN_SPEC_APPROVED` then `L4_DESIGN_QUALITY_APPROVED`, and its reviewed
implementation plan has ordered `L4_PLAN_SPEC_APPROVED` then
`L4_PLAN_QUALITY_APPROVED`. Shipped behavior remains unchanged until
implementation evidence and final reviews close.
...
reviews, and acceptance gates. The plan passed `L4_PLAN_SPEC_APPROVED` then
`L4_PLAN_QUALITY_APPROVED`; pre-implementation controls are next and no L4
...
**Use this when:** Configuring a generic LSP client or interpreting current dirty, pending, failed, invalidated, stale, initialization-failure, or no-watcher save behavior. L1, L2, L3, and L5 are shipped here; L4's accepted target is not yet shipped.
