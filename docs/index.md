...
**Description:** Active post-Stage-8 selector with two bounded series: Q0, Q1, and Q2 are complete; the Q3 design and reviewed implementation plan are accepted, and Q3 implementation is next. In the parallel L-series, L0 reliability/actionability, L1 authored symbols/signatures, and L2 recovery-safe static completion are complete; the L3 per-source entry-selection design gate is next, L4 remains sequenced behind it, and the owner-directed L5 authored-reference design is proposed for ordered review under the L-series reordering rule.
...
**Use this when:** Selecting or sequencing current post-Stage-8 work. [Q3 implementation](plans/2026-07-27-workflow-lisp-prompt-identity-diagnostics-implementation-plan.md) is next in Q, and the L3 per-source entry-selection design gate is next in L table order; the proposed L5 design may undergo review only under the recorded owner reordering and does not authorize implementation planning. L3 additionally requires proven compile-path reentrancy through MR-4 or an equivalent accepted fixture. Q3 remains unimplemented and Q4 remains blocked; do not infer an accepted L3 or L5 design. Do not select E0, the parked evolution roadmap, shelved parsimony candidates, or P1–P5 from this route.
...
**Description:** Revised proposed Stage-L5 authored-to-authored definition-index extension for prompt application heads plus feasibility-gated direct-retained `proc-ref` and macro heads. The exact projection joins original syntax to compiler identity fail closed; macro-consumed/specialized proc-refs defer, macro hits target their own `defmacro`, direct `(call ...)` navigation is already implemented regression coverage, and WCC/generated calls stay excluded.
**Keywords:** workflow-lisp, lsp, authored-references, go-to-definition, prompt-heads, proc-ref, macro-heads, compiler-retention
**Use this when:** Reviewing the revised proposed L5 design, its exact-span/identity projection, common freshness preflight, namespace/visibility cases, or retention gates. Independent specification review, then independent quality review, remain pending; the extension is not implemented, and no implementation plan may be written before both reviews approve and the conditional read-only feasibility gates close.
