...
**Use this when:** Executing ES Task 5 from reviewed candidate `d6fb50bc`, tree `77a53ff9`, after ordered `ES_TASK5_PLAN_SPEC_APPROVED` then `ES_TASK5_PLAN_QUALITY_APPROVED`; see the [plan review](../artifacts/review/es-task5-study-controller-plan-review.md). Task 1 is selected. The plan changes no DSL target, trial ledger/state, verdict, or public-result shape and authorizes no live provider call; Task 6 remains the exact scientific-lock owner-adoption gate.
