...
**Description:** Proposed owner-selected target-2.25 plan for bounded concurrent E1 `run-ref` arms, coordinator-owned single-writer evidence, blinded evaluation, deterministic checks, crash reconciliation, and a digest-bound verdict artifact.
**Keywords:** workflow-lisp, e-series, e2, trial, run-ref, concurrency, blinding, adjudication, component-plan
**Use this when:** Reviewing the proposed E2 execution contract and its authority reconciliation. Ordered `E2_PLAN_SPEC_APPROVED` then `E2_PLAN_QUALITY_APPROVED` are pending, so no E2 behavior exists and execution has not started. The historical registry/handle E2 proposal, E2O, C1-C3, and security/isolation work are not selected by this plan.
