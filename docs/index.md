...
**Description:** Active post-Stage-8 selector with two bounded series: Q0, Q1, and Q2 are complete, and the Q3 design-review gate is next. In the parallel L-series, L0 reliability/actionability and L1 authored symbols/signatures are complete; the L2 recovery-safe static completion design and component plan are accepted for implementation, with L3–L4 still sequenced and P1–P5 unselected.
...
**Use this when:** Selecting or sequencing current post-Stage-8 work. The Q3 design-review gate and reviewed L2 implementation execute independently under the one-Q/one-L concurrency and serialized shared-routing rule. Do not infer an accepted Q3 design or an implemented L2 surface before its plan closes. Do not select E0, the parked evolution roadmap, shelved parsimony candidates, or P1–P5 from this route.
...
**Current status:** Stages 6–8 and the selected target-2.18 list-traversal interstage are complete. Provider-supervision v1 closed Gate S7-v1 through `4d4f05c7`; cooperative peer messaging v1.1 closed Gate S7-v1.1 through `b08c04a6` plus Task 12's documentation, verification, and ordered `TASK12_FINAL_SPEC_APPROVED` / `TASK12_FINAL_QUALITY_APPROVED` reviews. The bounded structurally typed list surface is implemented under principle 29. Language-server v1 closed Gate S8 under its reviewed nine-task implementation plan. The numbered roadmap is historical and its post-Stage-8 handoff did not itself select successor work; the separate active language-quality roadmap records Q0–Q2 and L0–L1 complete, with Q3 at design review and L2 authorized for implementation under its accepted component plan.

**Current procedure-first substrate:** Native returns, typed guidance, the resolved-effect substrate, identity prerequisites, match-scoped scanner/record support, one reviewed internal pilot, generic resume projection-integrity hardening, the bounded migration wave, provider-supervision v1, cooperative peer messaging v1.1, bounded target-2.18 list traversal, prompt-calculus Q0–Q2, and language-server v1 plus its L0 reliability and L1 authored-symbol/signature tranches are implemented and gated. The wave intentionally retains 32 effect adapters and therefore does not establish universal procedure-first conversion. Its 63 YAML legacy rows completed the Stage-6 deletion queues; no numbered roadmap stage remains active. Current non-numbered execution routes Q-series work to Q3 design review and L-series work to the accepted L2 implementation plan; L2's recovery-safe static completion surface is not shipped until that plan closes.
...
**Use this when:** Configuring a generic LSP client or interpreting current dirty, pending, failed, invalidated, stale, initialization-failure, or no-watcher save behavior. L1 is shipped here; the guide also labels the accepted L2 recovery design as implementation-pending, while L3–L4 remain governed by later active-roadmap gates.
...
reviews. L1 is complete; L2's recovery-safe static completion design is
accepted and its reviewed implementation plan now owns execution.
...
**Description:** Accepted five-task TDD plan for one process-frozen form
...
**Use this when:** Executing or reviewing L2. The plan passed
`L2_PLAN_SPEC_APPROVED` then `L2_PLAN_QUALITY_APPROVED`; it does not mark the
recovery surface implemented before its final gate closes.
