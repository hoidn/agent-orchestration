...
**Use this when:** Executing ES Task 5 from reviewed candidate `d6fb50bc`, tree `77a53ff9`, after ordered `ES_TASK5_PLAN_SPEC_APPROVED` then `ES_TASK5_PLAN_QUALITY_APPROVED`; see the [plan review](../artifacts/review/es-task5-study-controller-plan-review.md). Tasks 1–6 are complete; deterministic synthesis closed at `32e4dc83`, tree `c843f06f`, after ordered `ES_TASK5_T6_SPEC_APPROVED` then `ES_TASK5_T6_QUALITY_APPROVED`, and Task 7 controller/CLI assembly is selected. The plan changes no DSL target, trial ledger/state, verdict, or public-result shape and authorizes no live provider call; the governing component-plan Task 6 remains the exact scientific-lock owner-adoption gate.
