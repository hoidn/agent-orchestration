...
**Use this when:** Checking the E-series shape, gates, and sequencing prerequisites, or tracing the historical proposal. E0 is implemented pending final gate: Task 1 landed at `b71bf62a`, Task 2 at `3d41a8bf`, and Task 3 at `3b934373`. Task 4 closed at `46387582`, tree `5dc787b6`, after ordered `E0_TASK4_SPEC_APPROVED` then `E0_TASK4_QUALITY_APPROVED`; its postcommit direct-routing control passed 74 tests. Task 5 final gate is in progress, so E0 is not complete. The canonical source is not copy-safe until `PASS_E0`, the accepted target-design examples remain non-copy-safe, and E1-E3 and C1-C3 remain Designed and unselected. E4P is exclusively absorbed by successor Stage Q3.
...
**Use this when:** Executing E0 under ordered `E0_PLAN_SPEC_APPROVED` then `E0_PLAN_QUALITY_APPROVED`. E0 is the sole selected tranche and is implemented pending final gate: Task 1 landed at `b71bf62a`, Task 2 at `3d41a8bf`, and Task 3 at `3b934373`. Task 4 closed at `46387582`, tree `5dc787b6`, after ordered `E0_TASK4_SPEC_APPROVED` then `E0_TASK4_QUALITY_APPROVED`; its postcommit direct-routing control passed 74 tests. Task 5 final gate is in progress, so E0 is not complete. The plan creates no new DSL target, runtime, loader, state, trial, child-run, controller, or historical evolution-selector machinery, and cannot select E1+.
