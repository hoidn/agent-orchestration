...
**Component-plan routing:** The refactoring, drain/G8 retirement, Stage-4 design, native returns, typed guidance, resolved-effect substrate, identity prerequisites, and [tracked-plan pilot](plans/2026-07-13-procedure-first-pilot-plan.md) are complete. Pilot evidence landed at `63e03330`, `e6a85cb7`, `de522c76`, `f5dbac88`, `76205d4f`, and `0769e837`; holistic specification and quality reviews approved HEAD `0769e837`. **The current selector is the [resume projection-integrity hardening design and planning plan](plans/2026-07-13-resume-projection-integrity-hardening-design-plan.md).** It designs/specifies the generic hardening contract and writes an implementation plan; it does not itself implement runtime hardening. Complete both it and the resulting implementation before the [reviewed migration waves](plans/2026-07-13-procedure-first-migration-waves-plan.md). Later routing remains [Stage 6 YAML retirement](plans/2026-07-07-yaml-retirement-program.md), [Stage 7 provider live binding](design/workflow_lisp_provider_live_binding.md), and [Stage 8 language server](design/workflow_lisp_language_server.md), in that order. The completed pilot is exactly one narrow evidence-only exception, not a general compatibility, promotion, family-wave, or YAML-retirement claim. The [Stage-0 activation plan](plans/2026-07-09-procedure-first-roadmap-activation-plan.md) is historical activation evidence.

**Current procedure-first substrate:** Native returns, typed guidance, the resolved-effect substrate, identity prerequisites, match-scoped scanner/record support, and one reviewed internal pilot are implemented and gated. The pilot retains exactly two completed dedicated runs, leaves historical clean artifact equality `not_asserted`, and preserves the route labels `leaf_runtime_candidate` / `migration_candidate` / `migration_evidence_only`. General cross-source resume compatibility, family migration waves, promotion, and YAML retirement remain unimplemented by that evidence.
