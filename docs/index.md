...
**Description:** Active post-Stage-8 selector with two bounded series: Q0 and Q1 are complete, while the Q2 design and reviewed implementation plan are accepted and implementation is active. In the parallel L-series, L0 reliability/actionability is implemented, while the L1 authored-symbol/signature design and reviewed implementation plan are accepted and implementation is active; L2–L4 remain sequenced without selecting P1–P5.
...
**Use this when:** Selecting or sequencing current post-Stage-8 work. Q2 and L1 reviewed implementation plans are active; follow the roadmap's one-Q/one-L concurrency and serialized shared-routing rule. Do not select E0, the parked evolution roadmap, shelved parsimony candidates, or P1–P5 from this route.
...
**Current status:** Stages 6–8 and the selected target-2.18 list-traversal interstage are complete. Provider-supervision v1 closed Gate S7-v1 through `4d4f05c7`; cooperative peer messaging v1.1 closed Gate S7-v1.1 through `b08c04a6` plus Task 12's documentation, verification, and ordered `TASK12_FINAL_SPEC_APPROVED` / `TASK12_FINAL_QUALITY_APPROVED` reviews. The bounded structurally typed list surface is implemented under principle 29. Language-server v1 closed Gate S8 under its reviewed nine-task implementation plan. The numbered roadmap is historical and its post-Stage-8 handoff did not itself select successor work; the separate active language-quality roadmap records Q0, Q1, and L0 complete, with Q2 and L1 accepted designs and reviewed implementation plans now active.

**Current procedure-first substrate:** Native returns, typed guidance, the resolved-effect substrate, identity prerequisites, match-scoped scanner/record support, one reviewed internal pilot, generic resume projection-integrity hardening, the bounded migration wave, provider-supervision v1, cooperative peer messaging v1.1, bounded target-2.18 list traversal, and language-server v1 plus its L0 reliability tranche are implemented and gated. The wave intentionally retains 32 effect adapters and therefore does not establish universal procedure-first conversion. Its 63 YAML legacy rows completed the Stage-6 deletion queues; no numbered roadmap stage remains active. Current non-numbered selection executes the reviewed Q2 and L1 implementation plans.
...
**Description:** Implemented `.orc` LSP v1 plus L0 reliability/actionability: a read-only stdio consumer of the production Stage-3 compiler with one-probe no-watcher reverse invalidation, structured initialization failures, visible compiler notes/expansion roles, content-keyed pure-projection export reuse, clean-open/save diagnostics, exact direct-call go-to-definition, document symbols, and visibility/registry completion under one canonical workspace root. L1 authored symbols/signatures have an accepted design and reviewed implementation plan but are not implemented; Task 1 is next.
**Keywords:** lisp-frontend, lsp, editor-tooling, diagnostics, go-to-definition, language-server, active-roadmap
**Use this when:** Auditing the implemented compiler/tooling contract. For installation, client initialization, editing behavior, freshness, and current limits, use the [Workflow Lisp Language Server Setup](workflow_lisp_language_server_setup.md). For L1–L4 navigation, recovery, entry-selection, and lifecycle sequencing, use the active roadmap's L-series.
...
**Use this when:** Configuring a generic LSP client or interpreting current dirty, pending, failed, invalidated, stale, initialization-failure, or no-watcher save behavior. L1–L4 remain governed by the active roadmap and must not be read back into this setup guide before their stage gates close.
...
**Description:** Accepted five-task L1 TDD plan for compiler-owned ten-kind
...
**Use this when:** Executing L1. The plan is accepted after
`L1_PLAN_SPEC_APPROVED` then `L1_PLAN_QUALITY_APPROVED`; it is routing, not
evidence that L1 behavior is already shipped.
