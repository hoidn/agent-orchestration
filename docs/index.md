...
**Description:** Active post-Stage-8 selector with two bounded series: Q0–Q3 are implemented and Q3 closes under its reviewed plan; Q4 still needs its concrete consumer; Q5 is partial through activation `bceb03e4` and remains stopped at Task 13. In the parallel L-series, L0 reliability/actionability, L1 authored symbols/signatures, L2 recovery-safe static completion, L3 immutable per-source entry selection, and owner-reordered L5 authored reference navigation are complete. MR-4 closed L3's compile-path-reentrancy prerequisite before implementation, and L3 completed over MR-4 under its reviewed plan. After accepted editor evidence and ordered design reviews plus a reviewed implementation plan, L4 diagnostic lifecycle and compile progress is implemented through current-only diagnostic publication and capability-gated progress; Neovim/docs Task 3 is `bdd1e822`. Task 4 focused 356 passed and broad comparison has zero new failures. The closure-only candidate now awaits preparatory Task 4 review and then final exact-tree review.
...
**Use this when:** Selecting or sequencing current post-Stage-8 work. Q4 remains gated on a bound concrete consumer. L4 next requires `L4_TASK4_SPEC_APPROVED` then `L4_TASK4_QUALITY_APPROVED`, followed after commit by `L4_FINAL_SPEC_APPROVED` then `L4_FINAL_QUALITY_APPROVED`; these are requirements, not issued outcomes. After L4 closes, Q5 requires the unissued `Q5_F1_F2_FIX_SPEC_APPROVED` then `Q5_F1_F2_FIX_QUALITY_APPROVED` over exact `492b1171`, followed by the unchanged combined invalid-then-valid real-provider gate. Do not substitute split proof or start Q5 Task 14. Do not select E0, the parked evolution roadmap, shelved parsimony candidates, or P1–P5 from this route.
...
**Current status:** Stages 6–8 and the selected target-2.18 list-traversal interstage are complete. Provider-supervision v1 closed Gate S7-v1 through `4d4f05c7`; cooperative peer messaging v1.1 closed Gate S7-v1.1 through `b08c04a6` plus Task 12's documentation, verification, and ordered `TASK12_FINAL_SPEC_APPROVED` / `TASK12_FINAL_QUALITY_APPROVED` reviews. The bounded structurally typed list surface is implemented under principle 29. Language-server v1 closed Gate S8 under its reviewed nine-task implementation plan. The active language-quality roadmap records Q0–Q3, L0–L3, and owner-reordered L5 complete. L4 diagnostic lifecycle/progress is implemented at `11629551` / `0d5f7009`; Neovim/docs Task 3 is `bdd1e822`; Task 4 focused 356 passed and its broad comparison has zero new failures. Its closure-only bytes await preparatory review, commit, and final exact-tree review. Q5 is partial through `bceb03e4`; stop `3fc3a09e` governs, correction `492b1171` is merged at `92515f98`, and required-but-unissued `Q5_F1_F2_FIX_SPEC_APPROVED` then `Q5_F1_F2_FIX_QUALITY_APPROVED` are next. Task 14 has not started, and no split proof satisfies the combined real-provider gate. Q4 remains blocked on its concrete consumer.
...
**Use this when:** Auditing the implemented compiler/tooling contract. For installation, client initialization, editing behavior, freshness, progress, and current limits, use the [Workflow Lisp Language Server Setup](workflow_lisp_language_server_setup.md). L4's behavior and Task 4 comparison are complete as evidence; the closure candidate still requires preparatory Task 4 review, commit, and final exact-tree review under the active roadmap.
...
selector reports 356 passed and the broad comparison has zero new failures. The
closure-only bytes require unissued `L4_TASK4_SPEC_APPROVED` then
`L4_TASK4_QUALITY_APPROVED`; after commit, unissued `L4_FINAL_SPEC_APPROVED`
then `L4_FINAL_QUALITY_APPROVED` exact-tree reviews remain required.
...
passed, and the broad comparison has zero new failures. The closure-only bytes require
unissued `L4_TASK4_SPEC_APPROVED` then `L4_TASK4_QUALITY_APPROVED`; after
commit, unissued `L4_FINAL_SPEC_APPROVED` then `L4_FINAL_QUALITY_APPROVED`
