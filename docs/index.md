...
**Description:** Tracked E-series program roadmap (incorporated 2026-07-30): records the accepted E0 [trial-runs design](design/workflow_lisp_trial_runs.md), its accepted [typed-program-gates companion](design/workflow_lisp_typed_program_gates.md), the now-satisfied lean-pilot Layer-0 owner-handoff prerequisite, and the E/M sequencing prerequisites. The designs passed ordered `E_DESIGNS_SPEC_APPROVED` then `E_DESIGNS_QUALITY_APPROVED`. The historical E0–E5 proposal is retained only as reference; the current canonical mapping is E0 direct control, E1 `run-ref`, E2 trials, and E3 external control.
...
**Use this when:** Checking the E-series shape, gates, and sequencing prerequisites, or tracing the historical proposal. E0 is complete at `fe7d6f9b`, tree `c20f6fd9`, with `PASS_E0` after ordered `E0_FINAL_SPEC_APPROVED` then `E0_FINAL_QUALITY_APPROVED`; its fresh postcommit control passed 115 tests. The canonical source is copy-safe only for the bounded one-call direct-task shape. E1 is eligible only for a separate owner activation decision; E1-E3 and C1-C3 remain Designed and unselected. E4P is exclusively absorbed by successor Stage Q3.
...
**Use this when:** Checking the accepted L6 scope, bounds, or planning entry condition. No implementation is selected. The proposed [L6 utility component plan](plans/2026-07-31-workflow-lisp-language-server-l6-utility-component-plan.md) is pending ordered review and selects nothing; an accepted plan plus explicit owner activation must name the independently selected unit or units. The completed language-quality roadmap is not reopened by this stage.
