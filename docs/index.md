...
**Description:** Current executable TDD selector for implementing the accepted resume projection-integrity contract. Runtime implementation and its acceptance evidence/reviews are pending.
**Keywords:** resume, projection, integrity, implementation-plan, current-selector, tdd
**Use this when:** Implementing the hardening before any procedure-first production migration wave.
...
**Component-plan routing:** The refactoring, drain/G8 retirement, Stage-4 design, native returns, typed guidance, resolved-effect substrate, identity prerequisites, and [tracked-plan pilot](plans/2026-07-13-procedure-first-pilot-plan.md) are complete. Resume projection-integrity artifact production and routing are complete: characterization landed at `1cd60767`, its [accepted design](design/resume_projection_integrity_hardening.md) at `52e2b05f`, its normative contract at `00135832`, and its reviewed implementation plan at `26a5d3db`; holistic routing reviews and fresh validation passed at closeout. **The current selector is the [resume projection-integrity hardening implementation plan](plans/2026-07-13-resume-projection-integrity-hardening-implementation-plan.md).** Runtime hardening remains pending; complete its acceptance evidence and implementation reviews before the [blocked migration waves](plans/2026-07-13-procedure-first-migration-waves-plan.md). Later routing remains [Stage 6 YAML retirement](plans/2026-07-07-yaml-retirement-program.md), [Stage 7 provider live binding](design/workflow_lisp_provider_live_binding.md), and [Stage 8 language server](design/workflow_lisp_language_server.md), in that order. The former early YAML-retirement window is closed; Stage 6 remains later unless a separately selected independent tranche is explicitly authorized. The completed pilot is exactly one narrow evidence-only exception, not a general compatibility, promotion, family-wave, or YAML-retirement claim. The [Stage-0 activation plan](plans/2026-07-09-procedure-first-roadmap-activation-plan.md) is historical activation evidence.

**Current procedure-first substrate:** Native returns, typed guidance, the resolved-effect substrate, identity prerequisites, match-scoped scanner/record support, and one reviewed internal pilot are implemented and gated. Resume projection-integrity hardening is designed, specified, and planned, but its runtime is not implemented. The pilot retains exactly two completed dedicated runs, leaves historical clean artifact equality `not_asserted`, and preserves the route labels `leaf_runtime_candidate` / `migration_candidate` / `migration_evidence_only`. General cross-source resume compatibility, family migration waves, promotion, and YAML retirement remain unimplemented by that evidence.
...
**Use this when:** Auditing the completed 2026-07-07 program or tracing its prerequisite history. For active execution, use the [Procedure-First Roadmap Execution Sequence](plans/2026-07-09-procedure-first-roadmap-execution-sequence.md); its current selector is the [resume projection-integrity hardening implementation plan](plans/2026-07-13-resume-projection-integrity-hardening-implementation-plan.md). Do not restart Phase 1.
