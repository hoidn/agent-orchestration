...
| Review the accepted future design for arbitrary strict-`Bool` `if` conditions, `cond`, and predicate-derived union proof | [Workflow Lisp Strict Boolean Control Flow](design/workflow_lisp_strict_boolean_control_flow.md) | Target-2.26 design only; current runnable source still follows the frontend specification and capability matrix. |
...
**Use this when:** Executing or auditing ES under reviewed candidate `27be07e2`, tree `e669471a`, after ordered `ES_PLAN_SPEC_APPROVED` then `ES_PLAN_QUALITY_APPROVED`; see the [plan review](../artifacts/review/es-first-effectiveness-study-plan-review.md). Tasks 0–5 are complete. Task 5's provider-free controller closed at `e5ec552d`, tree `1b0f0e67`, after ordered `ES_TASK5_SPEC_APPROVED` then `ES_TASK5_QUALITY_APPROVED`; see the [Task-5 review](../artifacts/review/es-first-effectiveness-study-task5-review.md). The owner-directed large-scope F1 refreeze is next. Its Tasks 1–8 subsume the old component Task 6 freeze/review/adoption work around one prospective `decision_lock.v3`; after that one owner adoption, existing ES Task 7 is next. No provider-bearing attempt, attempt ID, invalid-attempt allowance, or denominator row may be consumed before that gate. ES selects neither E3 implementation nor any superseded experiment substrate.

Historical ES tranche reviews remain at
[Task 1](../artifacts/review/es-first-effectiveness-study-task1-review.md),
[Task 2](../artifacts/review/es-first-effectiveness-study-task2-review.md),
[Task 3](../artifacts/review/es-first-effectiveness-study-task3-review.md), and
[Task 4](../artifacts/review/es-first-effectiveness-study-task4-review.md).

### [ES Task 5 Study Controller Execution Plan](plans/2026-08-03-es-task5-study-controller-execution-plan.md)
**Description:** Reviewed implementation-grade TDD sequence for the provider-free ES controller, beginning with a generic artifact-only projection of the exact E2 packet bytes at their existing freeze boundary, then private blinding joins, reviews, hard findings, accounting, synthesis, and public-entry integration.
**Keywords:** workflow-lisp, e-series, es, task-5, trial, packet-artifact, blinding, controller, synthesis, execution-plan
**Use this when:** Auditing completed ES Task 5. The provider-free public controller closed at `e5ec552d`, tree `1b0f0e67`, binary-diff SHA-256 `6bda9528a2bc97858d6208957a515b3832d8d380e0a466c5e673547e4a49b91e`, after ordered `ES_TASK5_SPEC_APPROVED` then `ES_TASK5_QUALITY_APPROVED`; see the [final Task-5 review](../artifacts/review/es-first-effectiveness-study-task5-review.md). The plan changes no DSL target, trial ledger/state, verdict, or public-result shape and made no live provider call. The owner-directed large-scope F1 refreeze is next; its single subsuming v3 gate and all live allocation remain blocked until it closes.

The Task-5 authority began with the ordered
[plan review](../artifacts/review/es-task5-study-controller-plan-review.md).
...
### [OMP Integration Design And Roadmap Extension](plans/2026-08-14-omp-integration-design-and-roadmap.md)
**Description:** Proposed tracked program (2026-08-14) for integrating the omp harness as a layer swap: a codex-parallel general/conf provider-template family, an `OMP_JSON_STDOUT` session codec feeding existing run-scoped tmux observation panes, a provider-agnostic prompt scaffolder whose `--output` path synthesizes and pins a validated output contract before generating and running ordinary editable `.orc`, the exact `--returns` escape hatch, shared `workflows/library/omp/` staging, named multiagent conf presets (`advised/`, `fanout/`, `peer-team/`, `advised-fanout/`), and a bidirectional session bridge (`omp --resume` on run evidence; session import back into orc). It extends and corrects the [omp integration proposal](plans/2026-08-14-omp-integration-proposal.md) (G1–G10 grounding table); the RPC interactive lane is consumer-gated; memory and cross-step hub state are banned by invariant.
**Keywords:** omp, provider, harness, session-transport, codec, scaffolder, prompt-asset, prompt-file, output-contract, typed-guidance, multiagent, advisor, fanout, observability, tmux, roadmap
**Active-roadmap route:** The E-series roadmap tracks this design as pending, unselected off-spine item `OMP-I1`, eligible only after frozen-ES hand-back and separate tranche-1 approval. Its F1–F8 checks are item-local and do not gate ES, E3, E-program closure, or P-series selection.
**Use this when:** Reviewing or executing the omp integration tranche; its literal/file prompt materialization or natural-language/exact output-contract authoring paths; choosing a multiagent conf preset; or auditing the admission tiers and hermeticity contract. Nothing is selected by listing: tranche 1 requires the owner approval recorded in the extension's Open Questions, feasibility gates F1–F8 are exit criteria for its first run, DP-A (2.16/2.17 deprecate-or-commit, drafting-guide guidance) waits for trial evidence, and nothing executes against the frozen ES apparatus.
