...
**Description:** Governing cross-plan work order for the completed refactor, migration, YAML-retirement, and provider-live-binding stages; it now routes the reviewed list-traversal implementation interstage before the final `.orc` language-server stage.
**Keywords:** workflow-lisp, roadmap, sequencing, parametric-types, procedure-first, refactoring, yaml-retirement
**Use this when:** Checking completed stage gates or selecting/auditing the active list-traversal implementation interstage and the eligible final language-server stage.
...
**Description:** Accepted target-2.18 design for total list construction/traversal, pure `list/map`, bounded effectful `list/map-effect`, whole-list-contract-expressible loop state, and typed containment-safe `path/join-under`, with record/union list elements, ProcRef mapping, and broader effect bodies deferred.
**Keywords:** lisp-frontend, lists, traversal, mapping, loop-recur, path-containment, principle-29
**Use this when:** Implementing or auditing the accepted design at `80df0616`; execute its independently reviewed implementation plan before Stage 8.
...
**Description:** Active reviewed seven-task TDD selector for target-2.18 dual-schema pure evaluation, list/path frontend forms, list-valued loop carriage, generic exhaustion diagnostics, effectful-map erasure, deterministic clean/resume evidence, and interstage closure.
**Keywords:** lisp-frontend, list-traversal, implementation-plan, target-2.18, pure-expr-schema-2, loop-resume
**Use this when:** Executing the selected post-Stage-7 interstage. It binds accepted design commit `80df0616` and received ordered `LIST_PLAN_SPEC_APPROVED` / `LIST_PLAN_QUALITY_APPROVED` plus final exact-byte reaffirmations.

### [Workflow Lisp Language Server](design/workflow_lisp_language_server.md)
**Description:** Accepted design at `cfcac27f` for an `.orc` LSP server: a stdio server that is a pure consumer of the existing compile entry points (per frontend spec §76.1), delivering diagnostics on save, go-to-definition, document symbols, and completion in v1, with error-tolerance, hover, and as-you-type checking deferred behind named frontend prerequisites.
**Keywords:** lisp-frontend, lsp, editor-tooling, diagnostics, go-to-definition, language-server
**Use this when:** Implementing or auditing the eligible final Stage 8. Execute only through the independently reviewed [nine-task implementation plan](plans/2026-07-25-workflow-lisp-language-server-implementation-plan.md), after the active list-traversal interstage closes.
...
**Use this when:** Auditing the completed 2026-07-07 program or tracing its prerequisite history. For sequencing, use the [Procedure-First Roadmap Execution Sequence](plans/2026-07-09-procedure-first-roadmap-execution-sequence.md); Stages 6-7 are complete, the reviewed list-traversal implementation plan is active, and Stage 8 is eligible. Do not restart Phase 1.
