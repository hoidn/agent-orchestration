...
**Description:** Accepted Stage 7 design for observation-only provider panes plus `with-live-providers`: exactly one worker and one supervisor run concurrently under a single-writer coordinator, and a validated `CONTINUE|STEER` union permits at most one fail-closed provider-session correction. The adverse T3a probe rejects direct `send-keys` steering.
**Keywords:** lisp-frontend, live-binding, tmux-observation, provider-supervision, turn-boundary-resume, structured-concurrency
**Use this when:** Reviewing or planning the bounded live-supervision direction after the adverse T3a result; the design is accepted and implementation remains gated on a reviewed Stage 7 execution plan.
...
**Use this when:** Auditing the completed 2026-07-07 program or tracing its prerequisite history. For sequencing, use the [Procedure-First Roadmap Execution Sequence](plans/2026-07-09-procedure-first-roadmap-execution-sequence.md); Stage 6 is complete and Stage 7 has an accepted design with implementation-plan review next. Do not restart Phase 1.
