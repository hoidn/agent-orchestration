...
**Description:** Active post-Stage-8 selector with two bounded series: Q0 and Q1 are complete, while the Q2 design and reviewed implementation plan are accepted and implementation is active. In the parallel L-series, L0 reliability/actionability and L1 authored symbols/signatures are complete; the L2 design-amendment/review gate is next, with L3–L4 still sequenced and P1–P5 unselected.
...
**Use this when:** Selecting or sequencing current post-Stage-8 work. Q2 implementation remains active while the L2 design-amendment/review gate is next; follow the roadmap's one-Q/one-L concurrency and serialized shared-routing rule. Do not infer an accepted L2 design. Do not select E0, the parked evolution roadmap, shelved parsimony candidates, or P1–P5 from this route.
...
**Current status:** Stages 6–8 and the selected target-2.18 list-traversal interstage are complete. Provider-supervision v1 closed Gate S7-v1 through `4d4f05c7`; cooperative peer messaging v1.1 closed Gate S7-v1.1 through `b08c04a6` plus Task 12's documentation, verification, and ordered `TASK12_FINAL_SPEC_APPROVED` / `TASK12_FINAL_QUALITY_APPROVED` reviews. The bounded structurally typed list surface is implemented under principle 29. Language-server v1 closed Gate S8 under its reviewed nine-task implementation plan. The numbered roadmap is historical and its post-Stage-8 handoff did not itself select successor work; the separate active language-quality roadmap records Q0, Q1, L0, and L1 complete, with Q2 implementation active and the L2 design-amendment/review gate next.

**Current procedure-first substrate:** Native returns, typed guidance, the resolved-effect substrate, identity prerequisites, match-scoped scanner/record support, one reviewed internal pilot, generic resume projection-integrity hardening, the bounded migration wave, provider-supervision v1, cooperative peer messaging v1.1, bounded target-2.18 list traversal, and language-server v1 plus its L0 reliability and L1 authored-symbol/signature tranches are implemented and gated. The wave intentionally retains 32 effect adapters and therefore does not establish universal procedure-first conversion. Its 63 YAML legacy rows completed the Stage-6 deletion queues; no numbered roadmap stage remains active. Current non-numbered selection keeps Q2 implementation active and routes L-series work next to L2 design review.
...
**Description:** Implemented target-2.20 Q1 prompt core covering importable `defprompt`, closed kind-based slots, fully applied named fills, prompt-owned results, one canonical compiled-fragment identity, schema-2.1 snapshots, and compatible completed-boundary resume. Q2 output positions have an accepted target-2.21 design and reviewed implementation plan but are not implemented; role-separated identity/comparison and judgment views remain separately owned by Q3–Q4.
**Keywords:** lisp-frontend, prompts, defprompt, slots, prompt-identity, judgments, principle-29
**Use this when:** Authoring or reviewing the bounded Q1 surface, which is available for authoring in target-2.20 `.orc`. It excludes partial application, runtime prompt values, call-site `:inputs`/`:prompt-dependencies`/`:returns`, output positions, judgments, and procedure/provider substitution.
...
**Description:** Accepted seven-task Q2 TDD plan for target-2.21 `:path :out`,
...
**Use this when:** Configuring a generic LSP client or interpreting current dirty, pending, failed, invalidated, stale, initialization-failure, or no-watcher save behavior. L1 is shipped here; L2–L4 remain governed by the active roadmap and must not be read back into this setup guide before their stage gates close.
...
reviews. L1 is complete; L2 begins only with its separate design-amendment and
review gate.
