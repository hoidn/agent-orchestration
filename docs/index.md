...
**Use this when:** Checking the E-series shape, gates, and sequencing prerequisites, or tracing the historical proposal. E0 is complete at `fe7d6f9b`, tree `c20f6fd9`, with `PASS_E0` after ordered `E0_FINAL_SPEC_APPROVED` then `E0_FINAL_QUALITY_APPROVED`; its fresh postcommit control passed 115 tests. The canonical source is copy-safe only for the bounded one-call direct-task shape. E1-E3 are owner-selected as of 2026-07-31: E1 Tasks 0–8 are complete under the [reviewed target-2.24 plan](plans/2026-07-31-workflow-lisp-e1-run-ref-component-plan.md) and [plan review](../artifacts/review/e1-run-ref-plan-review.md), so `run-ref` is implemented pending Task 9's final gate and is not yet promoted; E2 remains selected pending `PASS_E1` at target 2.25; E3 remains selected pending `PASS_E2` and review of the first fixed study. C1-C3 remain Designed and unselected. E4P is exclusively absorbed by successor Stage Q3.
...
**Use this when:** Executing E1 Task 9 or auditing its compiler-hermeticity, child-run, and end-to-end proofs. The plan was accepted at `0c392ac9`, tree `3a55ac5f`, after ordered `E1_PLAN_SPEC_APPROVED` then `E1_PLAN_QUALITY_APPROVED`; see the [plan review](../artifacts/review/e1-run-ref-plan-review.md). Tasks 0–8 are complete and target 2.24 `run-ref` is implemented pending the focused, broad non-security, ordered final-review, and canonical-exit gates. E2/E3 remain predecessor-gated.
