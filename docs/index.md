...
**Use this when:** Selecting or sequencing current post-Stage-8 work. Q4's consumer gate, ordered design review, and ordered implementation-plan review are complete, but implementation remains held until Q5 Task 14 closes and the consolidated lineage is transplanted to the canonical working tree. L4 completion requires fresh ordered `L4_FINAL_SPEC_APPROVED` then `L4_FINAL_QUALITY_APPROVED` against the corrected committed tree. The final verdict is an external exact-tree record; no repository byte can self-attest its own post-commit review. Q5's combined invalid-then-valid real-provider gate and ordered Task 13 reviews are complete; Task 14 closes only after the correction replay, final broad-delta adjudication, and fresh external `Q5_FINAL_SPEC_APPROVED` then `Q5_FINAL_QUALITY_APPROVED` over the final tree. Do not select E0, the parked evolution roadmap, shelved parsimony candidates, or P1–P5 from this route.
...
**Description:** Implemented target-2.23 Stage-Q5 surface under accepted design `872a29af` and reviewed plan `45468c55`. Tasks 1–13 are complete through `bb67f680`, including the combined invalid-then-valid real-provider consumer and its ordered specification/quality reviews. Stop `3fc3a09e` is retained as superseded historical provenance.
**Keywords:** lisp-frontend, prompts, phased-delivery, target-2.23, provider-attention, principle-30, turn-boundary
**Use this when:** Authoring or reviewing explicit phased delivery, its compatibility boundary, or its evidence/runtime machinery. The capability is implemented; Task 14's 10,919/42 broad result predates correction `5d8a3151` and is retained only as superseded candidate evidence. Closure still requires a fresh post-correction broad replay, exact delta adjudication, and external `Q5_FINAL_SPEC_APPROVED` then `Q5_FINAL_QUALITY_APPROVED` over the final exact tree. This routing entry does not self-attest those unissued final verdicts.
...
**Description:** Reviewed Q5 execution plan committed at `45468c55` after ordered `Q5_PLAN_SPEC_APPROVED` then `Q5_PLAN_QUALITY_APPROVED`; Tasks 1–13 are complete through `bb67f680`, and its 2026-07-28 Task 13 stop record at `3fc3a09e` is explicitly superseded historical provenance.
**Keywords:** lisp-frontend, prompts, phased-delivery, implementation-plan, target-2.23, q5
**Use this when:** Executing or auditing Q5’s reviewed order and ownership. Task 13 records the real attempt-10 pass, focused 2,245-pass gate, ordered reviews, and commit `bb67f680`. Task 14's recorded broad candidate predates correction `5d8a3151`; a fresh post-correction replay, exact delta adjudication, and external final exact-tree reviews remain required.
...
consumer is bound, its design is accepted, and its component plan has ordered
specification and quality approval. Implementation remains held until Q5 Task
14 closes and the consolidated `mr4-local` lineage is transplanted to the
canonical working tree.

### [Workflow Lisp Judgment Views](design/workflow_lisp_judgment_views.md)
**Description:** Accepted Q4 inspection-only design joining validated provider
...
target-2.22 generic-reviewer panel. The design is not implemented or available
for authoring; execute only through the
[reviewed implementation plan](plans/2026-07-29-workflow-lisp-judgment-views-implementation-plan.md).
Its Q5-closure, canonical-transplant, and M1 holds remain binding.

### [Workflow Lisp Judgment Views Implementation Plan](plans/2026-07-29-workflow-lisp-judgment-views-implementation-plan.md)
**Description:** Reviewed Q4 execution plan with one Task 0 entry gate followed
by nine implementation tasks for normative contracts, the exact export
compatibility gate, the generic WCC path-expression seam, atomic result
locators, persisted-surface contract resolution, closed reports, the panel
consumer, deterministic resume, bounded real use, and closure.
**Keywords:** lisp-frontend, judgments, provenance, implementation-plan, q4
**Use this when:** Executing Q4 after ordered `Q4_PLAN_SPEC_APPROVED` then
`Q4_PLAN_QUALITY_APPROVED`. Q5 Task-14 closure and canonical transplant must
complete first; M1 remains held on that same transplant.
...
present; Q4's consumer gate, design acceptance, and ordered implementation-plan
reviews are complete. Q5 Task-14 closure and canonical transplant still block
Q4 implementation.
