...
**Use this when:** Selecting or sequencing current post-Stage-8 work. Q4's consumer gate, ordered design review, and ordered implementation-plan review are complete; its implementation remains held while a Q5 real-provider attempt is live. L4 completion requires fresh ordered `L4_FINAL_SPEC_APPROVED` then `L4_FINAL_QUALITY_APPROVED` against the corrected committed tree. The final verdict is an external exact-tree record; no repository byte can self-attest its own post-commit review. After L4 closes, Q5 requires the unissued `Q5_F1_F2_FIX_SPEC_APPROVED` then `Q5_F1_F2_FIX_QUALITY_APPROVED` over exact `492b1171`, followed by the unchanged combined invalid-then-valid real-provider gate. Do not substitute split proof or start Q5 Task 14. Do not select E0, the parked evolution roadmap, shelved parsimony candidates, or P1–P5 from this route.
...
**Description:** Partial target-2.23 Stage-Q5 surface under accepted design `872a29af` and reviewed plan `45468c55`. Implementation is activated through `bceb03e4`, but the Task 13 stop at `3fc3a09e` remains authoritative and Task 14 has not started. F1/F2 evidence-surfacing correction `492b1171` is present through merge `92515f98`; `Q5_F1_F2_FIX_SPEC_APPROVED` then `Q5_F1_F2_FIX_QUALITY_APPROVED` are required and unissued.
**Keywords:** lisp-frontend, prompts, phased-delivery, target-2.23, provider-attention, principle-30, turn-boundary
**Use this when:** Reviewing Q5’s accepted additive call policy or its partial implementation. The exact `492b1171` fix bytes next require `Q5_F1_F2_FIX_SPEC_APPROVED` then `Q5_F1_F2_FIX_QUALITY_APPROVED`; both are required and unissued. Only then may the unchanged combined invalid-then-valid real-provider gate be re-attempted. Deterministic, synthetic, and opt-in live-probe evidence is not a split-proof substitute. `Q5_TASK13_SPEC_APPROVED` remains withheld.
...
**Description:** Reviewed Q5 execution plan committed at `45468c55` after ordered `Q5_PLAN_SPEC_APPROVED` then `Q5_PLAN_QUALITY_APPROVED`; implementation is activated through `bceb03e4` and its Task 13 stop record at `3fc3a09e` governs current execution.
**Keywords:** lisp-frontend, prompts, phased-delivery, implementation-plan, target-2.23, q5
**Use this when:** Executing or auditing Q5’s reviewed order and ownership. Q5 is partial: exact F1/F2 correction `492b1171` (merged at `92515f98`) next requires `Q5_F1_F2_FIX_SPEC_APPROVED` then `Q5_F1_F2_FIX_QUALITY_APPROVED`; both are required and unissued. After those reviews the unchanged combined real-provider gate remains mandatory. Task 13 is incomplete, Task 14 has not started, and no split proof closes the stop.
...
specification and quality approval. Implementation remains held while a Q5
real-provider attempt is live.
...
Its live-Q5 and M1 holds remain binding.
...
`Q4_PLAN_QUALITY_APPROVED`. Q5 live-attempt and M1 holds remain binding.
...
reviews are complete. The live-Q5 hold still blocks Q4 implementation.
