...
**Use this when:** Executing or auditing ES under reviewed candidate `27be07e2`, tree `e669471a`, after ordered `ES_PLAN_SPEC_APPROVED` then `ES_PLAN_QUALITY_APPROVED`; see the [plan review](../artifacts/review/es-first-effectiveness-study-plan-review.md). Tasks 0–5 are complete. Task 5's provider-free controller closed at `e5ec552d`, tree `1b0f0e67`, after ordered `ES_TASK5_SPEC_APPROVED` then `ES_TASK5_QUALITY_APPROVED`; see the [Task-5 review](../artifacts/review/es-first-effectiveness-study-task5-review.md). The owner-directed large-scope F1 refreeze is next, targeting a genuine 5,000–10,000 implementation-LOC controller-only reference product with tests and docs metered separately. It must close before Task 6 can freeze an adoptable package. No provider-bearing attempt, attempt ID, invalid-attempt allowance, or denominator row may be consumed before that refreeze and its exact owner-adoption gate. ES selects neither E3 implementation nor any superseded experiment substrate.
