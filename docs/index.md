...
**Description:** Proposed implementation-grade TDD sequence for the provider-free ES controller, beginning with a generic artifact-only projection of the exact E2 packet bytes at their existing freeze boundary, then private blinding joins, reviews, hard findings, accounting, synthesis, and public-entry integration.
**Keywords:** workflow-lisp, e-series, es, task-5, trial, packet-artifact, blinding, controller, synthesis, execution-plan
**Use this when:** Reviewing or executing ES Task 5 after its ordered plan verdicts. The plan changes no DSL target, trial ledger/state, verdict, or public-result shape and authorizes no live provider call; Task 6 remains the exact scientific-lock owner-adoption gate.
