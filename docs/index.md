...
**Use this when:** Checking the E-series shape, gates, and sequencing prerequisites, or tracing the historical proposal. The accepted designs remain non-copy-safe because their surfaces are not implemented. ML closure and the lean-pilot owner-decision handoff are complete. The reviewed component plan now selects E0 only; Task 1 is next and no E0 behavior is implemented yet. E1-E3 and C1-C3 remain unselected. E4P is exclusively absorbed by successor Stage Q3.

### [E0 Canonical Direct-Control Component Plan](plans/2026-07-31-workflow-lisp-e0-direct-control-component-plan.md)
**Description:** Reviewed, selected E0-only component plan for one target-2.23 library workflow with typed task/model/effort inputs, exactly one composed provider boundary, a direct `Bool` result, completed-result reuse, and accounting-parity conformance against an ordinary one-provider workflow.
**Keywords:** workflow-lisp, e-series, e0, direct-control, provider, accounting, component-plan
**Use this when:** Executing E0 from Task 1 under ordered `E0_PLAN_SPEC_APPROVED` then `E0_PLAN_QUALITY_APPROVED`. E0 is the sole selected tranche and no implementation exists yet. The plan creates no new DSL target, runtime, loader, state, trial, child-run, controller, or historical evolution-selector machinery, and cannot select E1+.
