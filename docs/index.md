...
**Description:** Active post-Stage-8 selector with two bounded series: Q0, Q1, and Q2 are complete; the Q3 design and reviewed implementation plan are accepted, and Q3 implementation is next. In the parallel L-series, L0 reliability/actionability, L1 authored symbols/signatures, and L2 recovery-safe static completion are complete; owner-reordered L5 has an accepted design and closed feasibility gates, with its admitted-shapes implementation plan at ordered review, while L3 remains queued behind that plan gate and L4 remains sequenced behind L3.
...
**Use this when:** Selecting or sequencing current post-Stage-8 work. [Q3 implementation](plans/2026-07-27-workflow-lisp-prompt-identity-diagnostics-implementation-plan.md) is next in Q. In L, review the [proposed L5 implementation plan](plans/2026-07-27-workflow-lisp-l5-authored-reference-navigation-implementation-plan.md): it admits prompt heads and only final unexpanded direct-retained proc-refs in non-generated, non-specialized authored owners, while macro heads defer shape-wide. L5 implementation remains unauthorized until ordered plan reviews approve. L3 remains queued and additionally requires proven compile-path reentrancy through MR-4 or an equivalent accepted fixture. Q3 remains unimplemented and Q4 remains blocked; do not infer an accepted L3 design or shipped L5 behavior. Do not select E0, the parked evolution roadmap, shelved parsimony candidates, or P1–P5 from this route.
...
**Current status:** Stages 6–8 and the selected target-2.18 list-traversal interstage are complete. Provider-supervision v1 closed Gate S7-v1 through `4d4f05c7`; cooperative peer messaging v1.1 closed Gate S7-v1.1 through `b08c04a6` plus Task 12's documentation, verification, and ordered `TASK12_FINAL_SPEC_APPROVED` / `TASK12_FINAL_QUALITY_APPROVED` reviews. The bounded structurally typed list surface is implemented under principle 29. Language-server v1 closed Gate S8 under its reviewed nine-task implementation plan. The numbered roadmap is historical and its post-Stage-8 handoff did not itself select successor work; the separate active language-quality roadmap records Q0–Q2 and L0–L2 complete, with the Q3 design and reviewed implementation plan accepted and Q3 implementation next while L3 remains at its separate design-review gate.

**Current procedure-first substrate:** Native returns, typed guidance, the resolved-effect substrate, identity prerequisites, match-scoped scanner/record support, one reviewed internal pilot, generic resume projection-integrity hardening, the bounded migration wave, provider-supervision v1, cooperative peer messaging v1.1, bounded target-2.18 list traversal, prompt-calculus Q0–Q2, and language-server v1 plus its L0 reliability, L1 authored-symbol/signature, and L2 recovery-static-completion tranches are implemented and gated. The wave intentionally retains 32 effect adapters and therefore does not establish universal procedure-first conversion. Its 63 YAML legacy rows completed the Stage-6 deletion queues; no numbered roadmap stage remains active. Current non-numbered execution routes Q-series work to Q3 implementation under its accepted reviewed plan and L-series work to the L3 per-source entry-selection design gate, whose entry requires proven compile-path reentrancy.
...
**Description:** Implemented target-2.20 Q1 prompt core and target-2.21 Q2 output positions: importable `defprompt`, closed kind-based slots, fully applied named fills, one `:path :out` role, prompt-owned results, exact v1/v2 compiled-fragment identity/carriage, schema-2.1 snapshots, state-atomic file-plus-structured validation, and compatible completed-boundary resume. The Q3 role-separated identity/comparison design is accepted but unimplemented; Q4 judgment views remain separately owned.
**Keywords:** lisp-frontend, prompts, defprompt, slots, prompt-identity, judgments, principle-29
**Use this when:** Authoring or reviewing the bounded Q1/Q2 surfaces, available for authoring at targets 2.20 and 2.21 respectively. Q2 permits one `:path :out` slot to own prompt rendering plus a required UTF-8 file postcondition alongside one structured result. The surface still excludes partial application, runtime prompt values, call-site `:inputs`/`:prompt-dependencies`/`:returns`, optional or dynamic output sets, judgments, and procedure/provider substitution.

### [Workflow Lisp Phased Contract Delivery](design/workflow_lisp_phased_contract_delivery.md)
**Description:** Proposed target-2.23 Stage-Q5 design for explicit phased delivery of one canonical fragment prompt: it preserves compiled Q1/Q2 identity, requires landed/accepted Q3 attempt evidence but no Q4 judgment surface, and gives a dedicated single-attempt coordinator exact canonical-versus-actual identity/reporting, a proof-authoritative closed start outcome with one handle-free provider-cleanup evidence union, exact active-handle validation before projecting the unchanged target-2.17 adapter proof, inert pre-start binding/locator values and post-start endpoint binding, one deadline, full Q2 validation, T2 cleanup-pending/finished terminalization, natural-proof-before-ledger joined-pending-commit publication, total reason-equal diagnostics, and ledger-only digest validation. Independent specification review, then quality review, remain pending.
**Keywords:** lisp-frontend, prompts, phased-delivery, target-2.23, provider-attention, principle-30, turn-boundary
**Use this when:** Reviewing Q5’s additive call policy, Q3/report version gate, closed-outcome deadline-aware production-adapter prerequisite, capability refusal, byte cut, materialization retry, handle-bound-proof versus handle-free-evidence projection, pre-start zero-resource and post-start endpoint-allocation boundary, T0–T4 failure grammar, diagnostic projection, offline digest categories, endpoint shutdown, post-join failure, or sticky interruption contract. Omitted/composed calls remain exact; explicit phased delivery is unavailable and fail-closed. Pre-planning proves only adapter feasibility, including explicit failed-start allocation/cleanup evidence and exact active-handle proof validation; coordinator invalid-then-valid and terminal-resource behavior remain implementation completion gates. The separate `prompt-queue` proposal is not a dependency.
...
Task-7 capture incident, and ordered final reviews. Q2 is shipped; current
Q-series selection starts with Q3 implementation under its accepted reviewed
plan.

### [Workflow Lisp Prompt Identity Diagnostics](design/workflow_lisp_prompt_identity_diagnostics.md)
**Description:** Accepted target-2.22 Q3 design for content-free,
...
**Use this when:** Implementing the accepted
[Q3 plan](plans/2026-07-27-workflow-lisp-prompt-identity-diagnostics-implementation-plan.md).
The Q3 design and reviewed implementation plan are accepted, implementation is
next, and Q3 remains unimplemented; acceptance does not expose target-2.22
authoring or unblock Q4.
...
**Description:** Accepted reviewed execution plan for target-2.22 Q3 compiler
...
**Use this when:** Executing Q3 next under its task-by-task TDD and ordered
review gates. The plan is accepted at `ad5474c7`, implementation has not
started, capability remains Designed, and Q4 stays blocked on Q3 completion.

### [Workflow Lisp Language Server](design/workflow_lisp_language_server.md)
**Description:** Implemented `.orc` LSP v1 plus L0 reliability/actionability, L1 authored symbols/signatures, and L2 recovery-safe static completion: a read-only stdio consumer of the production Stage-3 compiler with one-probe no-watcher reverse invalidation, structured initialization failures, visible compiler notes/expansion roles, content-keyed pure-projection export reuse, clean-open/save diagnostics, exact direct-call go-to-definition, compiler-owned ten-kind document symbols, and namespace-preserving callable/form completion under one canonical workspace root. L2 now gives valid recovery states only the process-frozen form rows with `isIncomplete=true` and no stale callable; current success remains full and closed states remain empty.
**Keywords:** lisp-frontend, lsp, editor-tooling, diagnostics, go-to-definition, language-server, active-roadmap
**Use this when:** Auditing the implemented compiler/tooling contract. For installation, client initialization, editing behavior, freshness, and current limits, use the [Workflow Lisp Language Server Setup](workflow_lisp_language_server_setup.md). For the completed L1/L2 surfaces and L3–L4 entry-selection/lifecycle sequencing, use the active roadmap's L-series.

### [Workflow Lisp LSP Authored Reference Navigation](design/workflow_lisp_lsp_authored_reference_navigation.md)
**Description:** Accepted Stage-L5 authored-to-authored definition-index design for prompt application heads plus the feasibility-admitted narrow direct-retained `proc-ref` shape. The exact projection joins original syntax to compiler catalog identity fail closed; macro heads defer shape-wide, macro-consumed/erased/generated-owner/specialized-owner proc-refs remain null, direct `(call ...)` navigation is already implemented regression coverage, and WCC/generated calls stay excluded.
**Keywords:** workflow-lisp, lsp, authored-references, go-to-definition, prompt-heads, proc-ref, macro-heads, compiler-retention
**Use this when:** Reviewing the accepted L5 design, its exact-span/identity projection, common freshness preflight, namespace/visibility cases, or closed feasibility disposition. The [proposed implementation plan](plans/2026-07-27-workflow-lisp-l5-authored-reference-navigation-implementation-plan.md) still requires ordered specification then quality review; the extension is not implemented.
...
**Description:** Proposed six-task TDD plan for collision-safe five-field reference rows, exact prompt-head and narrowly retained proc-ref joins, visibility and full preflight/null matrices, a repository-real review-workflow stdio gate, and durable baseline/routing closure.
**Keywords:** lisp-frontend, lsp, authored-references, prompt-heads, proc-ref, implementation-plan, tdd
**Use this when:** Reviewing the L5 implementation boundary and execution order. The plan includes only feasibility-admitted prompt heads and direct-retained unexpanded proc-refs in non-generated, non-specialized authored owners; macro heads, erased/macro-consumed/specialized proc-refs, new direct-call behavior, WCC/generated shapes, and compiler/frontend changes remain excluded. Implementation awaits `L5_PLAN_SPEC_APPROVED` then `L5_PLAN_QUALITY_APPROVED`.
...
**Use this when:** Configuring a generic LSP client or interpreting current dirty, pending, failed, invalidated, stale, initialization-failure, or no-watcher save behavior. L1 and L2 are shipped here; L3–L4 remain governed by later active-roadmap gates.
