...
**Description:** Governing cross-plan work order for the completed refactor, migration, YAML-retirement, and provider-live-binding stages; it now routes the bounded list-traversal review before or alongside planning the final `.orc` language-server stage.
**Keywords:** workflow-lisp, roadmap, sequencing, parametric-types, procedure-first, refactoring, yaml-retirement
**Use this when:** Checking completed stage gates or selecting the immediate list-traversal review and the eligible final language-server stage.
...
**Description:** Owner-directed final proposal for total list construction/traversal, pure `list/map`, bounded effectful `list/map-effect`, list-typed loop state, and containment-safe `path/join-under`, with ProcRef mapping and higher-order/lambda scope deferred.
**Keywords:** lisp-frontend, lists, traversal, mapping, loop-recur, path-containment, principle-29
**Use this when:** Performing the selected post-Stage-7 independent design review. Review commit `bef65fdf` under Principle 29, then produce one small implementation plan before or alongside Stage-8 planning.
...
**Use this when:** Auditing the completed 2026-07-07 program or tracing its prerequisite history. For sequencing, use the [Procedure-First Roadmap Execution Sequence](plans/2026-07-09-procedure-first-roadmap-execution-sequence.md); Stages 6-7 are complete, the bounded list-traversal review is selected, and Stage 8 is eligible. Do not restart Phase 1.
