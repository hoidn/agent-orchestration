...
**Description:** Active first reviewed-internal-identity-retirement pilot. Architect Path A selected Task 1A, but the pilot source remains unchanged and Task 2 is blocked pending corrected pre-edit scan regeneration, owner attestations, quiescence, isolation, and immutable evidence.
...
**Description:** Active corrective prerequisite whose generic scanner and record-layer changes landed at `e43461f9` and `5f382401`: match-scoped nonterminal and query-derived old-consumer counts gate eligibility, while terminal matching counts and required whole-store totals remain evidence only.
**Keywords:** procedure-first, retirement, state-store, matching-counts, evidence
**Use this when:** Reviewing the corrected store-query semantics or continuing pre-edit scan regeneration to the pilot's owner-attestation boundary.
