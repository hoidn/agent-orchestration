...
**Description:** Active first reviewed-internal-identity-retirement pilot. Task 1A's immutable pre-edit evidence is committed at `63e03330`; Task 2's one-phase migration and compile-only characterization landed and passed two-stage review at `e6a85cb7` without a runtime run. The frozen baseline is unchanged. Task 3 is selected under the owner-approved exact-two-run protocol; runtime evidence has not started.
