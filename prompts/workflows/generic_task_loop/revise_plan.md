...
Task:
Revise the current plan and check strategy to resolve the blocking findings from the plan review.
...
- Update the check strategy so it credibly explains what runnable verification should exist by the implementation loop.
...
- Produce an updated `check_strategy` artifact.
...
- Do not introduce hidden-evaluator assumptions into the visible verification strategy.
