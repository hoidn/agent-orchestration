...
175. At-least-once interrupted-visit recovery: `orchestrate resume` validates an exact ordinary, session, supervision, peer-group, or phased in-flight visit; preserves older same-name terminal results; discards only partial visit authority; allocates fresh identities; and emits exactly one `provider_attempt_interrupted_rerun`. Malformed or ambiguous state fails before provider launch, and compatible completed-result reuse invokes no provider.
176. Provider-session recovery observability: report/status surfaces expose bounded interrupted-rerun context and step-level `provider_session` summaries without printing raw metadata transport to console stdout; absent or torn audit evidence alone does not block a validated recovery.
...
185. v2.11 adjudication mismatch recovery: logical deadlines and candidate/evaluator retry scopes remain unchanged; fully consistent completed state reuses without invocation, while one exact run-owned adjudication mismatch discards only that partial visit and reruns with `adjudication_state_mismatch_rerun`. Unknown, ambiguous, or escaping cleanup authority fails closed before mutation or provider launch.
...
193. v2.14 variant output: `variant_output` validates a tagged-union JSON bundle, exposes the discriminant, any declared `shared_fields`, and only the selected variant's fields, rejects duplicate artifact names or JSON pointers across the discriminant, shared fields, and fields in the active variant while allowing reuse across distinct variant-only branches, rejects forbidden fields, and injects provider prompt contracts while validating command outputs without prompt injection
