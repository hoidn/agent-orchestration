...
| 2.15 (unreleased private preview) | Direct JSON root results (`output_bundle`/`variant_output` fields with `json_pointer: ""`) and widened public workflow `outputs` (`kind: collection`, i.e. `optional\|list\|map`) for Workflow Lisp-compiled workflows | Native transportable returns wave 1 (`docs/design/workflow_lisp_native_transportable_returns.md`) is landed compiler-internally: `VERSION_ORDER` accepts `"2.15"` but `SUPPORTED_VERSIONS` does not, so only the Workflow Lisp compiler's own shared-validation call enables it (`WorkflowLoader._enabled_preview_versions`). Ordinary loader entrypoints (CLI `run`/`resume`/`report`, dashboard projection, `build.py` imported-bundle manifest loading) continue to reject `version: "2.15"`. Existing v2.14 record/union contracts, artifact names, source identities, and checkpoint identities are unaffected and non-regressive. Promotion (moving `"2.15"` into `SUPPORTED_VERSIONS`) waits for the dependent typed-result-guidance wave 2, which adds `(result T ...)`, field annotations, and the guidance wire schema. |
