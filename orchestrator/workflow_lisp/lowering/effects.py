...
def _lower_command_result_operation(
    ...
    *,
    ...
    binding = context.command_boundary_environment.bindings_by_name.get(command_result.step_name)
    if binding is None:
        raise lowering_core._compile_error(
            code="command_result_tool_invalid",
            message=f"unknown command boundary `{command_result.step_name}` during lowering",
