...
class RunProjector:
        ...
        for entry in snapshot_steps:
            ...
            steps.append(
                DashboardStep(
                    ...
                    file_refs=self._artifact_file_refs(artifacts, resolver, warnings),
                )
            )
        return steps
    ...
    def _availability(self, run_root: Path) -> dict[str, bool]:
        ...
        return {
            "prompt_audits": logs.exists() and any(logs.glob("*.prompt.txt")),
            "stdout": logs.exists() and any(logs.glob("*.stdout")),
            "stderr": logs.exists() and any(logs.glob("*.stderr")),
            "provider_sessions": provider_sessions.exists() and any(provider_sessions.iterdir()),
            "state_backups": any(run_root.glob("state.json.*.bak")),
        }
