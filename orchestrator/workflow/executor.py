...
from .call_frame_state import (
    _managed_jobs_config_from_step,
    _thaw_workflow_value,
)
...
from .adjudication_runner import AdjudicationBindings, AdjudicationRunner
...
class WorkflowExecutor:
    ...
    def __init__(
        ...
        self._adjudication_runner = AdjudicationRunner(
            AdjudicationBindings(
                ...
                prompt_composer=lambda: self.prompt_composer,
                current_step=lambda: self.current_step,
                resume_mode=lambda: self.resume_mode,
                max_retries=lambda: self.max_retries,
                retry_delay_ms=lambda: self.retry_delay_ms,
                global_secrets=lambda: self.global_secrets,
                ...
                create_provider_context=lambda *args, **kwargs: (
                    self._create_provider_context(*args, **kwargs)
                ),
                prepare_provider_invocation=lambda *args, **kwargs: (
                    self.provider_executor.prepare_invocation(*args, **kwargs)
                ),
                substitute_provider_params=lambda *args, **kwargs: (
                    self.provider_executor._substitute_params(*args, **kwargs)
                ),
                execute_provider_invocation=lambda *args, **kwargs: (
                    self._execute_provider_invocation(*args, **kwargs)
                ),
                persist_control_flow_state=lambda state: (
                    self._persist_control_flow_state(state)
                ),
                provider_env_with_runtime_output_bundle_path=lambda *args, **kwargs: (
                    self._provider_env_with_runtime_output_bundle_path(*args, **kwargs)
                ),
                resolve_output_contract_paths=lambda *args, **kwargs: (
                    self._resolve_output_contract_paths(*args, **kwargs)
                ),
                substitute_path_template=lambda *args, **kwargs: (
                ...
                prepare_candidate_workspace_from_baseline=lambda *args, **kwargs: (
                    prepare_candidate_workspace_from_baseline(*args, **kwargs)
                ),
                select_candidate=lambda *args, **kwargs: select_candidate(*args, **kwargs),
                materialize_run_score_ledger=lambda *args, **kwargs: (
                    materialize_run_score_ledger(*args, **kwargs)
                ),
    ...
    def _legacy_frontend_index(self) -> CompiledFrontendIndex:
        ...
        if not isinstance(index, CompiledFrontendIndex):
            index = CompiledFrontendIndex(None)
            self._frontend_index = index
        return index
    ...
    @_compiled_frontend_node_origins.setter
    def _compiled_frontend_node_origins(self, value) -> None:
        self._legacy_frontend_index().node_origins = value

    @property
    def _compiled_frontend_step_origins(self):
        return self._legacy_frontend_index().step_origins

    @_compiled_frontend_step_origins.setter
    def _compiled_frontend_step_origins(self, value) -> None:
        self._legacy_frontend_index().step_origins = value

    @property
    def _compiled_frontend_command_boundaries(self):
        return self._legacy_frontend_index().command_boundaries

    @_compiled_frontend_command_boundaries.setter
    def _compiled_frontend_command_boundaries(self, value) -> None:
        self._legacy_frontend_index().command_boundaries = value

    def _load_compiled_frontend_step_origins(
        self,
        provenance: Any,
    ) -> Dict[str, Mapping[str, Any]]:
        """Compatibility delegator for tests that bind the legacy loader."""
        return self._legacy_frontend_index()._load_step_origins(provenance)
    ...
    def _load_compiled_frontend_node_origins(
        ...
        """Compatibility delegator for tests that bind the legacy loader."""
        return self._legacy_frontend_index()._load_node_origins(provenance)
    ...
    def _load_compiled_frontend_command_boundaries(
        ...
        """Compatibility delegator for tests that bind the legacy loader."""
        return self._legacy_frontend_index()._load_command_boundaries(provenance)
    ...
    def _compiled_frontend_origin_for_step(
        ...
        *,
        ...
        """Compatibility delegator for legacy source-origin lookup."""
        return self._legacy_frontend_index().origin_for_step(
            step_name, step_id, node_id=node_id
        )
    ...
    def _compiled_frontend_command_boundary_for_step(
        ...
        *,
        ...
        boundary = self._legacy_frontend_index().command_boundary_for_step(
            step_name, step_id, node_id=node_id
        )
    ...
    def _emit_compiled_frontend_step_display(
        ...
        *,
        ...
        index._emit_step_display_with_boundary(
            step_name,
            step_id,
            node_id=node_id,
            boundary=boundary,
            stream_output=getattr(self, "stream_output", False),
    ...
    def _execute_adjudicated_provider_with_context(
        ...
        """Compatibility delegator to the adjudication runner."""
        return self._adjudication_runner.execute_adjudicated_provider_with_context(
            step,
            context,
            state,
            runtime_step_id=runtime_step_id,
    ...
    def _adjudication_consumed_artifacts_for_prompt(
        ...
        *,
        ...
        """Compatibility delegator for directly bound prompt-evidence tests."""
        from .adjudication_scoring import adjudication_consumed_artifacts_for_prompt
    ...
    def _execute_resource_transition(
        ...
        *,
        ...
        return execute_resource_transition(self, step, state, scope=scope)
    ...
    def _execute_pure_projection(
        ...
        *,
        ...
        from .steps.pure_projection import execute_pure_projection

        return execute_pure_projection(self, step, state, scope=scope)
    ...
    def _execute_materialize_view(
        ...
        *,
        ...
        return execute_materialize_view(
            self,
            step,
            state,
            scope=scope,
            render_view_fn=render_view,
        )
