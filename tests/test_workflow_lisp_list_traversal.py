...
def test_unknown_target_versions_still_fail_closed(tmp_path: Path) -> None:
    with pytest.raises(LispFrontendCompileError) as excinfo:
        build_syntax_module(
            read_sexpr_text(
                _module_source("2.21"),
                source_path="target_221_list_traversal.orc",
            )
        )
    ...
    result = validate_workflow_mapping(
        WorkflowMappingBuildRequest(
            ...
            workflow_path=tmp_path / "target-221.orc",
            frontend_kind="workflow_lisp",
        ),
        options=WorkflowMappingValidationOptions(
            workspace_root=tmp_path,
    ...
    assert any("Unsupported version '2.21'" in error.message for error in result.errors)
