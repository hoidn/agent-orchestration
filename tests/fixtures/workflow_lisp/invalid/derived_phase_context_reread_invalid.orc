(workflow-lisp
  (:language "0.1")
  (:target-dsl "2.14")
  (defmodule derived_phase_context_reread_invalid)
  ...
  (defworkflow run-entry
    ((work_item_bootstrap WorkItemBootstrapSeed)
     ...
     (baseline_design_path BaselineDesignDoc)
     (progress_ledger_path ProgressLedger)
     (manifest_path StateExisting))
    -> FixtureResult
    (call bootstrap-reread-path
  ...
  (defworkflow bootstrap-reread-path
    ((phase-ctx PhaseCtx)
     (work_item_bootstrap WorkItemBootstrapSeed)
     (steering_path SteeringDoc)
     (target_design_path TargetDesignDoc)
      ...
      (let* ((item-ctx
             ...
             (selection
               (record WorkItemSelectionPayload
                 ...
                 :baseline_design_path baseline_design_path
                 :progress_ledger_path progress_ledger_path)))
        (call run-with-reread-path
          :item-ctx item-ctx
          :selection selection
          :manifest_path manifest_path))))

  (defworkflow run-with-reread-path
    ((item-ctx ItemCtx)
