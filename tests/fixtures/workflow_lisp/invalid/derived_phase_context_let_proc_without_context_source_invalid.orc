(workflow-lisp
  (:language "0.1")
  (:target-dsl "2.14")
  (defmodule derived_phase_context_let_proc_without_context_source_invalid)
  ...
  (defworkflow bootstrap-phase-root
    ((phase-ctx PhaseCtx)
     (work_item_bootstrap WorkItemBootstrapSeed)
     (steering_path SteeringDoc)
     (target_design_path TargetDesignDoc)
      ...
      (let* ((item-ctx
             ...
             (selection
               (record WorkItemSelectionPayload
                 ...
                 :baseline_design_path baseline_design_path
                 :progress_ledger_path progress_ledger_path)))
        (run-outer item-ctx selection))))

  (defproc run-outer
    ((item-ctx ItemCtx)
     (selection WorkItemSelectionPayload))
    -> FixtureResult
    :effects ((calls-workflow lisp_frontend_design_delta/plan_phase::run-plan-phase))
    :lowering inline
    (let-proc (run-selection-only ((selection WorkItemSelectionPayload)) -> FixtureResult
                :captures ()
                (let* ((plan
                         (call run-plan-phase
                           ...
                           :progress_report_target_path selection.resolved_inputs.progress_report_target_path
                  ...
                  (match plan
                    ...
                    ((EXHAUSTED exhausted)
                     (variant FixtureResult BLOCKED
                       :reason "exhausted")))))
      (invoke-selection-only selection (proc-ref run-selection-only))))

  (defproc invoke-selection-only
    ((selection WorkItemSelectionPayload)
     (run-item ProcRef[(WorkItemSelectionPayload) -> FixtureResult]))
    -> FixtureResult
    :effects ()
    :lowering inline
    (run-item selection))
