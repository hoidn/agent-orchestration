(workflow-lisp
  (:language "0.1")
  (:target-dsl "2.14")
  (defmodule derived_phase_context_ambiguous)
  ...
  (defworkflow run-entry
    ((work_item_bootstrap WorkItemBootstrapSeed)
     ...
     (progress_ledger_path ProgressLedger))
    -> FixtureResult
    (call bootstrap-ambiguous-item-ctx
  ...
  (defworkflow bootstrap-ambiguous-item-ctx
    ((phase-ctx PhaseCtx)
     (work_item_bootstrap WorkItemBootstrapSeed)
     (steering_path SteeringDoc)
     (target_design_path TargetDesignDoc)
      ...
      (let* ((item-ctx
             ...
             (selection
               (record WorkItemSelectionPayload
                 ...
                 :baseline_design_path baseline_design_path
                 :progress_ledger_path progress_ledger_path)))
        (call run-with-ambiguous-derived-source
          :item-ctx item-ctx
          :sibling-item-ctx sibling-item-ctx
          :selection selection))))

  (defworkflow run-with-ambiguous-derived-source
    ((item-ctx ItemCtx)
     (sibling-item-ctx ItemCtx)
