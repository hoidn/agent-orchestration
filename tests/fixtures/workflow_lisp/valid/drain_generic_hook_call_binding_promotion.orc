(workflow-lisp
  (:language "0.1")
  (:target-dsl "2.14")
  (defmodule drain_generic_hook_call_binding_promotion)
  ...
  (defunion MinimalGapResult
    ...
    (BLOCKED
      ...
      (blocker-class BlockerClass)))
  (defworkflow select-source
    ((ctx MinimalDrainCtx))
    -> MinimalSelection
    (command-result drain_select
      :argv ("python" "scripts/select_next_item.py" ctx.manifest)
      :returns MinimalSelection))
  (defproc select-with-call-binding
    ((ctx MinimalDrainCtx))
    -> MinimalSelection
    :effects ((calls-workflow drain_generic_hook_call_binding_promotion::select-source)
              (uses-command drain_select))
    :lowering inline
    (let* ((selection (call select-source :ctx ctx)))
      selection))
  (defproc run-minimal
  ...
  (defworkflow minimal-backlog-drain
    ...
    (let* ((terminal (backlog-drain-proc
                       ctx
                       (proc-ref select-with-call-binding)
                       ...
                       (__generated-relpath-seed__
                         ...
                         "drain_generic_hook_call_binding_promotion_progress_report_seed"))))
