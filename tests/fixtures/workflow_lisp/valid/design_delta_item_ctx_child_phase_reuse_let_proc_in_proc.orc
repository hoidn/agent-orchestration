(workflow-lisp
  (:language "0.1")
  (:target-dsl "2.14")
  (defmodule design_delta_item_ctx_child_phase_reuse_let_proc_in_proc)
  ...
  (defproc invoke-run-item
    ((item-ctx ItemCtx)
     ...
     (run-item ProcRef[(ItemCtx WorkItemSelectionPayload) -> FixtureResult]))
    -> FixtureResult
    :effects ()
    :lowering inline
  ...
  (defproc run-outer
    ...
    :lowering inline
        (let-proc (run-item-ctx-first ((item-ctx ItemCtx) (selection WorkItemSelectionPayload)) -> FixtureResult
                    :captures ()
                    (let* ((selection-ctx
