(workflow-lisp
  (:language "0.1")
  (:target-dsl "2.14")
  (defmodule phase_stdlib_run_provider_phase)
  ...
  (defworkflow run-provider-phase-demo
    ((phase-ctx PhaseCtx)
  ...
  (defworkflow produce-one-of-demo
    ((phase-ctx PhaseCtx)
     (inputs ImplementationInputs))
    -> ImplementationSummary
    (with-phase phase-ctx implementation
                 ...
                 :candidates
                   ((COMPLETED
                      (execution_report_path
                        :target (phase-target execution-report)
                        :schema WorkReport))
                    (BLOCKED
                      (progress_report_path
                        :target (phase-target progress-report)
                        :schema WorkReport)
                      (blocker_class BlockerClass
