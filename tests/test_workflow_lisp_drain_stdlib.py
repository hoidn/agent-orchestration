...
VALID_STDLIB_RICH_GAP_FIXTURE = (
    FIXTURES / "valid" / "drain_stdlib_backlog_drain_rich_gap_payload.orc"
)
...
def test_compile_stage3_module_carries_rich_gap_payload_through_generic_drain(
    tmp_path: Path,
) -> None:
    ...
    repeat_step = next(step for step in parent.authored_mapping["steps"] if "repeat_until" in step)
    body_steps = list(_iter_nested_steps(repeat_step["repeat_until"]["steps"]))
    gap_drafter_call = next(
        step for step in body_steps if step.get("call", "").endswith("gap-draft.v1")
    )
    gap_write_root_name = next(
        name for name in gap_drafter_call["with"] if name.startswith("__write_root__")
    )

    bindings = gap_drafter_call["with"]
    assert set(bindings) == {
        "ctx__run__run-id",
    ...
    for field_name in ("work-item-id", "plan-target-path", "architecture-path"):
        ref = bindings[f"gap__{field_name}"]["ref"]
        assert "selector-run.artifacts" in ref
        assert ref.endswith(f"return__gap__{field_name}")
    assert "gap-draft__managed_write_roots.artifacts" in bindings[gap_write_root_name]["ref"]
