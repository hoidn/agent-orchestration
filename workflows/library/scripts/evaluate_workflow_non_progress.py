...
def _recommended_focus(trigger_codes: list[str]) -> str:
    ...
    if "same_blocker_repeated" in trigger_codes or "same_work_item_repeatedly_blocked" in trigger_codes:
        return "Reassess the blocker: continue the loop's own recovery machinery, stop for external review, or escalate for a human decision."
    ...
    if "plan_churn_without_outcome_change" in trigger_codes:
        return "Stop revising the plan incrementally; stop for external review or escalate for a human decision."
