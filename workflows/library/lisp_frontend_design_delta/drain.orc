(workflow-lisp
  ...
  (defworkflow build-drain-runtime-owned
    ((run RunCtx))
    -> DrainRuntimeOwned
    (let* ((bridged-run-state
             (call carry-drain-run-state-bridge)))
      (record DrainRuntimeOwned
        :run (record RunCtx
               :run-id run.run-id
               :state-root run.state-root
               :artifact-root run.artifact-root)
        :run_state_path bridged-run-state.run_state_path)))
    ...
    (let* ((ignored-architecture-bundle-path
           ...
           (ctx
             (record DesignDeltaDrainCtx
               ...
               :run_state_path runtime-owned.run_state_path
  ...
  (defworkflow carry-drain-run-state-bridge
    ((run_state_path StateExisting))
    -> DrainRunStateBridge
    (record DrainRunStateBridge
