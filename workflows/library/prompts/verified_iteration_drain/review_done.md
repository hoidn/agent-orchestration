...
Return `APPROVE` or `REJECT` as the typed provider result, and write the same
value to the compatibility file named by `done_review_decision_path` in the
consumed work order. When rejecting, append each unmet criterion and
