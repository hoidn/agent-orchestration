...
- the roadmap does not define the project's completion boundary: requested outcome, in-scope behavior, artifacts, interfaces, data, workflows, quality constraints, compatibility promises, required consumers, and evidence for completion
...
- the roadmap treats a subset, pilot, representative path, or intermediate milestone as project-complete without accounting for remaining in-scope work at the same semantic level as the brief
- the roadmap uses narrowing labels such as "selected", "practical", "representative", "initial", or "workhorse" as the final definition of done without the brief explicitly setting that narrower goal
