...
- Allowed decisions are `APPROVE`, `REVISE`, `ESCALATE_REPLAN`, `ESCALATE_ROADMAP_REVISION`, and `BLOCK`.
...
- Use `ESCALATE_ROADMAP_REVISION` when the selected-tranche scope boundary itself must be reduced, split, moved, rechartered, or given different prerequisites.
...
- identify material design, plan, or scope-boundary requirements that were deferred without roadmap-level authority, rationale, and handoff criteria
...
- diagnose the root cause of any nontrivial runtime failures systematically: reproduce the failure, locate the faulty component, and fix the underlying cause rather than the symptom
...
Write `APPROVE`, `REVISE`, `ESCALATE_REPLAN`, `ESCALATE_ROADMAP_REVISION`, or `BLOCK` to the `implementation_review_decision` path specified in the Output Contract.
...
Use `ESCALATE_ROADMAP_REVISION` when the correct resolution is to reduce, split, move, recharter, or reprioritize selected-tranche scope rather than repair the current design or plan locally.
...
- no claimed, plan-required, or scope-boundary-required tasks remain unimplemented
- no unfinished prerequisite makes the delivered behavior unsafe, misleading, or unusable
- no material design or scope-boundary requirement was silently dropped or deferred without roadmap-level authority
