...
Do not satisfy hidden/system-owned context, generated path, checkpoint, or boundary findings by fabricating context records or hard-coded generated paths in authored source. First look for the approved binding surface and use it when available. Report a blocker only when that surface is absent, contradictory, or would require changing the approved contract.
