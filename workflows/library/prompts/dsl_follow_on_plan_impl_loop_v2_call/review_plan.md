...
Write the review as markdown to the workspace-relative path stored in `state/follow-on-plan-phase/plan_review_report_path.txt`.
Write `APPROVE` or `REVISE` to `state/follow-on-plan-phase/plan_review_decision.txt`.
