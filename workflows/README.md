...
| `workflows/examples/neurips_steered_backlog_drain.yaml` | Downstream reference; reusable call-based; input-required | `2.7` | `neurips-steered-backlog-drain` | Steered backlog drain wrapper copied from PtychoPINN: builds a backlog manifest, applies a deterministic roadmap gate, recovers in-progress items, drafts missing authorized backlog items, and calls the reusable NeurIPS selected-item stack. |
