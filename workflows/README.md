...
| `workflows/examples/major_project_tranche_drain_stack_v2_call.yaml` | Reusable call-based | `2.7` | `major-project-tranche-drain-stack-v2-call` | Full major-project drain driver: calls the roadmap phase once, then loops over manifest selection, one selected tranche stack call, and manifest status update until all tranches are complete or the queue is blocked. |
| `workflows/examples/major_project_tranche_drain_from_manifest_v2_call.yaml` | Reusable call-based; input-required | `2.7` | `major-project-tranche-drain-from-manifest-v2-call` | Continuation driver for an already-approved roadmap: consumes an existing project brief, project roadmap, and tranche manifest, then starts directly at the manifest-drain loop. |
...
| `workflows/library/tracked_big_design_phase.yaml` | `2.7` | `tracked-big-design-phase` | Reusable big-design phase for one selected tranche: consumes project roadmap context, drafts a self-contained tranche design, and runs tracked review/revise with explicit `BLOCK`. |
| `workflows/library/major_project_tranche_design_plan_impl_stack.yaml` | `2.7` | `major-project-tranche-design-plan-impl-stack` | Reusable major-project tranche stack: runs tracked big design, then reuses the generic tracked plan and implementation phases without forking those interfaces. |
| `workflows/library/major_project_tranche_plan_impl_from_approved_design_stack.yaml` | `2.7` | `major-project-tranche-plan-impl-from-approved-design-stack` | Reusable major-project tranche continuation stack: start from an existing approved design, then reuse the generic tracked plan and implementation phases while preserving the standard item summary and outcome shape. |
