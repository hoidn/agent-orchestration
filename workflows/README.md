...
- For a one-off `.orc` review/fix loop over design docs, inspect
...
| `workflows/examples/review_revise_parametric_design_docs.orc` | Workflow Lisp one-off review/fix workflow; input-required | `2.14` | `review-revise-parametric-design-docs` | One-off `.orc` workflow that runs a bounded review/fix loop over the Workflow Lisp review/revise stdlib integration, structural parametric constraints, and compile-time parametric specialization design docs. Use it as the current model for targeted design-doc review/fix loops, not as a production drain or general YAML parity replacement. |
