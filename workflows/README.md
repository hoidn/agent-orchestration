...
| `workflows/library/major_project_tranche_drain_iteration.yaml` | `2.12` | `major-project-tranche-drain-iteration` | One major-project drain iteration: selects a tranche, runs the selected tranche stack, updates the manifest for terminal tranche outcomes, or promotes roadmap-revision outputs for advisory `APPROVE`, `REVISE`, or `BLOCK` decisions. |
