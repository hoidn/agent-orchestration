...
| `workflows/examples/major_project_tranche_continue_from_approved_design_v2_call.yaml` | Reusable call-based; input-required | `2.7` | `major-project-tranche-continue-from-approved-design-v2-call` | One-tranche major-project continuation driver: selects the next ready tranche from an existing manifest, reuses an already-approved design, runs plan plus implementation, and updates the manifest without rerunning big design. |
...
| `workflows/library/major_project_tranche_plan_phase.yaml` | `2.7` | `major-project-tranche-plan-phase` | Major-project-local plan phase: consumes upstream escalation context, reviews plans with `APPROVE`, `REVISE`, `ESCALATE_REDESIGN`, or `BLOCK`, and emits plan escalation context. |
| `workflows/library/major_project_tranche_implementation_phase.yaml` | `2.7` | `major-project-tranche-implementation-phase` | Major-project-local implementation phase: tracks cumulative implementation review iterations, injects threshold context, and lets review return `APPROVE`, `REVISE`, `ESCALATE_REPLAN`, or `BLOCK`. |
| `workflows/library/major_project_roadmap_revision_phase.yaml` | `2.7` | `major-project-roadmap-revision-phase` | Major-project-local roadmap revision phase: consumes a structured roadmap change request and revises the approved roadmap plus tranche manifest narrowly around that request. |
| `workflows/library/major_project_tranche_drain_iteration.yaml` | `2.7` | `major-project-tranche-drain-iteration` | One major-project drain iteration: selects a tranche, runs the selected tranche stack, updates the manifest for terminal tranche outcomes, or dispatches roadmap revision for `ESCALATE_ROADMAP_REVISION`. |
| `workflows/library/major_project_tranche_design_plan_impl_stack.yaml` | `2.7` | `major-project-tranche-design-plan-impl-stack` | Reusable major-project tranche stack: runs tracked big design, major-project-local planning, and major-project-local implementation with upward rerouting for replan, redesign, and roadmap revision. |
| `workflows/library/major_project_tranche_plan_impl_from_approved_design_stack.yaml` | `2.7` | `major-project-tranche-plan-impl-from-approved-design-stack` | Reusable major-project tranche continuation stack: starts from an existing approved design, then uses the major-project-local plan and implementation phases while preserving item summary/outcome shape. |
